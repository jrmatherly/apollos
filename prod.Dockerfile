...
RUN --mount=type=cache,target=/root/.cache/uv \
    ...
    uv sync --frozen --no-install-project --no-dev --extra prod --no-build-isolation-package openai-whisper
...
ARG PORT
EXPOSE ${PORT}
ENTRYPOINT ["gunicorn", "-c", "gunicorn-config.py", "src.apollos.main:app"]
