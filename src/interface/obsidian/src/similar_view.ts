...
export class ApollosSimilarView extends ApollosPaneView {
	...
	getViewType(): string {
		return ApollosView.SIMILAR;
	}

	getDisplayText(): string {
		return "Apollos Similar Documents";
	}
	...
	async onOpen() {
		...
		const mainContainerEl = contentEl.createDiv({
			cls: "apollos-similar-container",
		});

		// Create search input container
		const searchContainerEl = mainContainerEl.createDiv({
			cls: "apollos-similar-search-container",
		});

		// Create search input
		this.searchInputEl = searchContainerEl.createEl("input", {
			cls: "apollos-similar-search-input",
			attr: {
				type: "text",
		...
		const refreshButtonEl = searchContainerEl.createEl("button", {
			cls: "apollos-similar-refresh-button",
		});
	...
	/**
	 * Get similar documents from Apollos API
	 *
	...
	async getSimilarDocuments(query: string): Promise<SimilarResult[]> {
		...
		try {
			...
			// Setup Query Apollos backend for search results
			let encodedQuery = encodeURIComponent(query);
			let searchUrl = `${this.setting.apollosUrl}/api/search?q=${encodedQuery}&n=${this.setting.resultsCount}&r=true&client=obsidian`;
			let headers = {
				Authorization: `Bearer ${this.setting.apollosApiKey}`,
			};

			// Get search results from Apollos backend
			...
			let results = data
				.filter((result: any) => {
					...
					if (
						this.currentFile &&
						result.additional.file.endsWith(this.currentFile.path)
					) {
						return false;
					}
					return true;
	...
	renderResults(results: SimilarResult[]) {
		...
		this.resultsContainerEl.createEl("div", {
			cls: "apollos-results-count",
			text: `Found ${results.length} similar document${results.length > 1 ? "s" : ""}`,
		});

		// Create results list
		const resultsListEl = this.resultsContainerEl.createEl("div", {
			cls: "apollos-similar-results-list",
		});

		// Render each result
		results.forEach(async (result) => {
			const resultEl = resultsListEl.createEl("div", {
				cls: "apollos-similar-result-item",
			});
			...
			const headerEl = resultEl.createEl("div", {
				cls: "apollos-similar-result-header",
			});

			// Show filename with appropriate color
			const fileEl = headerEl.createEl("div", {
				cls: `apollos-result-file ${result.inVault ? "in-vault" : "not-in-vault"}`,
			});
			...
			const contentEl = resultEl.createEl("div", {
				cls: "apollos-result-entry apollos-similar-content-hidden",
			});
			...
			let entry_snipped_indicator =
				result.entry.split("\n").length > lines_to_render
					? " **...**"
					: "";
			let snipped_entry = result.entry
				.split("\n")
				.slice(0, lines_to_render)
				.join("\n");
			...
			moreContextButton.addEventListener("click", (e) => {
				...
				if (
					contentEl.classList.contains(
						"apollos-similar-content-hidden",
					)
				) {
					contentEl.classList.remove(
						"apollos-similar-content-hidden",
					);
					contentEl.classList.add("apollos-similar-content-visible");
					moreContextButton.empty();
					moreContextButton.createSpan({ text: "Less context" });
					setIcon(moreContextButton.createSpan(), "chevron-up");
				} else {
					contentEl.classList.remove(
						"apollos-similar-content-visible",
					);
					contentEl.classList.add("apollos-similar-content-hidden");
					moreContextButton.empty();
					moreContextButton.createSpan({ text: "More context" });
					setIcon(moreContextButton.createSpan(), "chevron-down");
				}
			...
			resultEl.addEventListener("click", (e) => {
				...
				if (
					e.target === moreContextButton ||
					moreContextButton.contains(e.target as Node)
				) {
					return;
				}
	...
	async openResult(result: SimilarResult) {
		...
		const binaryFiles = this.app.vault
			.getFiles()
			.filter((file) =>
	...
	updateUI(
		...
		const messageEl = this.resultsContainerEl.createEl("div", {
			cls: "apollos-similar-message",
		});
		...
		switch (state) {
			...
			case "no-file":
				messageEl.setText(
					"No file is currently open. Open a markdown file to see similar documents.",
				);
				break;
			case "unsupported-file":
				messageEl.setText(
					"This file type is not supported. Only markdown files are supported.",
				);
				break;
			case "no-results":
			...
			case "error":
				messageEl.setText(
					`Error: ${message || "Failed to fetch similar documents"}`,
				);
				break;
			case "empty-query":
				messageEl.setText(
					"Please enter a search query or open a markdown file.",
				);
				break;
		}
	}
