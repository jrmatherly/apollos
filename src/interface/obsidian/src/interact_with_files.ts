...
/**
 * Interface representing the result of parsing a Apollos edit block
 */
...
/**
 * Class that handles file operations for the Apollos plugin
 */
export class FileInteractions {
	private app: App;
	private readonly EDIT_BLOCK_START = "<apollos_edit>";
	private readonly EDIT_BLOCK_END = "</apollos_edit>";
	...
	private getRecentActiveMarkdownFiles(N: number): TFile[] {
		...
		const recentActiveFiles = this.app.workspace
		...
		console.log(
			`Using ${recentActiveFiles.length} recently viewed md files for context: ${recentActiveFiles.map((file) => file.path).join(", ")}`,
		);
	 ...
	 */
	public async getOpenFilesContent(
		...
		const recentFiles = this.getRecentActiveMarkdownFiles(
			this.CONTEXT_FILES_LIMIT,
		);
...
`;
	...
	public findBestMatchingFile(
		...
		for (const file of files) {
			...
			const distanceWithExt = this.levenshteinDistance(
				targetName.toLowerCase(),
				file.name.toLowerCase(),
			);
			const distanceWithoutExt = this.levenshteinDistance(
				targetName.toLowerCase(),
	...
	public parseEditBlock(
		...
		try {
			...
			if (!isComplete) {
				...
				const searchStartMatch = cleanContent.match(
					/<<<<<<< SEARCH\n([\s\S]*)/,
				);
				if (searchStartMatch) {
					partialData.find = searchStartMatch[1];
					if (!partialData.file) {
						// If file not on first line, try line before SEARCH
			...
			if (editData && !editData.file) {
				error = {
					...
					details:
						'The "file" field is required and should contain the target file name',
				};
			} else if (
				editData &&
				(editData.find === undefined || editData.find === null)
			) {
				error = {
					type: "missing_field",
					message: 'Missing "find" field markers',
					details:
						'The "find" field is required. It should contain the content to find in the file or be empty for new files',
				};
				...
				error = {
					...
					details:
						'The "replace" field is required. It should contain the content to replace or be empty to indicate deletion',
				};
			}
	...
	public parseEditBlocks(message: string): EditBlock[] {
		...
		const editBlockRegex = new RegExp(
			`${this.EDIT_BLOCK_START}([\\s\\S]*?)${this.EDIT_BLOCK_END}`,
			"g",
		);

		let match;
		while ((match = editBlockRegex.exec(message)) !== null) {
			const { editData, cleanContent, error } = this.parseEditBlock(
				match[1],
			);
	 ...
	 */
	public createPreviewWithDiff(
		originalText: string,
		newText: string,
	): string {
		// Define unique tokens to temporarily replace existing formatting markers
		const HIGHLIGHT_TOKEN = "___APOLLOS_HIGHLIGHT_MARKER___";
		const STRIKETHROUGH_TOKEN = "___APOLLOS_STRIKETHROUGH_MARKER___";
		...
		const preserveFormatting = (text: string): string => {
			...
			let processed = text.replace(
				/==(.*?)==/g,
				`${HIGHLIGHT_TOKEN}$1${HIGHLIGHT_TOKEN}`,
			);
			// Replace existing strikethrough markers with non-greedy pattern
			processed = processed.replace(
				/~~(.*?)~~/g,
				`${STRIKETHROUGH_TOKEN}$1${STRIKETHROUGH_TOKEN}`,
			);
			return processed;
		};
		...
		const restoreFormatting = (text: string): string => {
			...
			let processed = text.replace(
				new RegExp(HIGHLIGHT_TOKEN + "(.*?)" + HIGHLIGHT_TOKEN, "g"),
				"==$1==",
			);
			// Restore strikethrough markers
			processed = processed.replace(
				new RegExp(
					STRIKETHROUGH_TOKEN + "(.*?)" + STRIKETHROUGH_TOKEN,
					"g",
				),
				"~~$1~~",
			);
		...
		const minLength = Math.min(
			preservedOriginal.length,
			preservedNew.length,
		);
		while (
			prefixLength < minLength &&
			preservedOriginal[prefixLength] === preservedNew[prefixLength]
		) {
			prefixLength++;
		}
		...
		while (
			...
			preservedOriginal[preservedOriginal.length - 1 - suffixLength] ===
				preservedNew[preservedNew.length - 1 - suffixLength]
		) {
			suffixLength++;
		}
		...
		const commonSuffix = preservedOriginal.slice(
			preservedOriginal.length - suffixLength,
		);
		const originalDiff = preservedOriginal.slice(
			prefixLength,
			preservedOriginal.length - suffixLength,
		);
		const newDiff = preservedNew.slice(
			prefixLength,
			preservedNew.length - suffixLength,
		);
	...
	private processSingleEdit(
		...
		if (findText === "") {
			...
			startIndex = currentFileContent.indexOf(
				findText,
				frontmatterEndIndex,
			);
			if (startIndex !== -1) {
		...
		const textToReplace = currentFileContent.substring(
			startIndex,
			endIndex,
		);
	...
	public async applyEditBlocks(
		...
		const files = this.getRecentActiveMarkdownFiles(
			this.CONTEXT_FILES_LIMIT,
		);
		...
		for (const block of editBlocks) {
			try {
				...
				const currentContent = currentFileContents.get(
					targetFile.path,
				)!;
				...
				const frontmatterEndIndex = frontmatterMatch
					? frontmatterMatch[0].length
					: 0;

				const processedEdit = this.processSingleEdit(
					block.find,
					block.replace,
					currentContent,
					frontmatterEndIndex,
				...
				currentFileContents.set(
					targetFile.path,
					processedEdit.newContent,
				);
			} catch (error) {
				validationResults.push({
					block,
		...
		try {
			...
			for (const result of validationResults) {
				...
				if (processedEdit.error) {
					throw new Error(
						`Failed to re-locate edit markers for file "${targetFile.basename}" during application. Content may have shifted.`,
					);
				}
				...
				editResults.push({
					block: { ...block, replace: processedEdit.preview },
					success: true,
				});
			}
		} catch (error) {
	...
	public transformEditBlocks(message: string): string {
		...
		for (const block of partialBlocks) {
			...
			const { editData, cleanContent, error, inProgress } =
				this.parseEditBlock(content, isComplete);
			if (!editData && !error) {
			...
			if (error) {
				// Error block
				console.error("Error parsing apollos-edit block:", error);
				console.error("Content causing error:", content);
				...
				htmlRender = `<details class="apollos-edit-accordion error">
                    <summary>${errorTitle}</summary>
                    <div class="apollos-edit-content">
                        <p class="apollos-edit-error-message">${errorDetails}</p>
                        <pre><code class="language-md error">${diffContent}</code></pre>
                    </div>
                </details>`;
			} else if (editData && inProgress) {
				// In-progress block
				htmlRender = `<details class="apollos-edit-accordion in-progress">
                    <summary>📄 ${editData.file} <span class="apollos-edit-status">In Progress</span></summary>
                    <div class="apollos-edit-content">
				...
				const targetFile = this.findBestMatchingFile(
					editData.file,
					files,
				);
				const displayFileName = targetFile
					? `${targetFile.basename}.${targetFile.extension}`
					: editData.file;

				htmlRender = `<details class="apollos-edit-accordion success">
			...
			if (isComplete) {
				transformedMessage = transformedMessage.replace(
					`${this.EDIT_BLOCK_START}${content}${this.EDIT_BLOCK_END}`,
					htmlRender,
				);
			} else {
				transformedMessage = transformedMessage.replace(
					`${this.EDIT_BLOCK_START}${content}`,
					htmlRender,
				);
			}
		}
