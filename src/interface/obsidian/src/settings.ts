...
import {
	canConnectToBackend,
	fetchChatModels,
	fetchUserServerSettings,
	getBackendStatusMessage,
	updateContentIndex,
	updateServerChatModel,
} from "./utils";
...
export interface ApollosSetting {
	resultsCount: number;
	apollosUrl: string;
...
// NOTE: Default URL below is hardcoded. If forking this project, update apollosai.dev to your domain.
export const DEFAULT_SETTINGS: ApollosSetting = {
	resultsCount: 15,
	apollosUrl: "https://app.apollosai.dev",
	apollosApiKey: "",
...
export class ApollosSettingTab extends PluginSettingTab {
	plugin: Apollos;
	...
	constructor(app: App, plugin: Apollos) {
		super(app, plugin);
		this.plugin = plugin;
	}
	...
	display(): void {
		...
		// Add notice whether able to connect to apollos backend or not
		let backendStatusMessage = getBackendStatusMessage(
			...
			this.plugin.settings.apollosUrl,
			this.plugin.settings.apollosApiKey,
		);

		const connectHeaderEl = containerEl.createEl("h3", {
			title: backendStatusMessage,
		});
		const connectHeaderContentEl = connectHeaderEl.createSpan({
			cls: "apollos-connect-settings-header",
		});
		const connectTitleEl = connectHeaderContentEl.createSpan({
			text: "Connect",
		});
		const backendStatusEl = connectTitleEl.createSpan({
			text: this.connectStatusIcon(),
			cls: "apollos-connect-settings-header-status",
		});
		if (
			...
			if (this.plugin.settings.userInfo.photo) {
				...
				profilePicEl.addEventListener("click", () => {
					new Notice(backendStatusMessage);
				});
			} else if (this.plugin.settings.userInfo.email) {
				const initial =
					this.plugin.settings.userInfo.email[0].toUpperCase();
				const profilePicEl = connectHeaderContentEl.createDiv({
					text: initial,
					cls: "apollos-profile apollos-profile-initial",
				});
				profilePicEl.addEventListener("click", () => {
					new Notice(backendStatusMessage);
				});
		...
		if (
			...
			connectHeaderEl.title =
				this.plugin.settings.userInfo?.email === "default@example.com"
					? "Signed in"
					: `Signed in as ${this.plugin.settings.userInfo.email}`;
		}

		// Add apollos settings configurable from the plugin settings tab
		const apiKeySetting = new Setting(containerEl)
			.setName("Apollos API Key")
			.addText((text) =>
				text
					.setValue(`${this.plugin.settings.apollosApiKey}`)
					.onChange(async (value) => {
						this.plugin.settings.apollosApiKey = value.trim();
						({
							connectedToBackend:
								this.plugin.settings.connectedToBackend,
		...
		apiKeySetting.descEl.createEl("span", {
			text: "Connect your Apollos Cloud account. ",
		});
		apiKeySetting.descEl.createEl("a", {
			text: "Get your API Key",
			href: `${this.plugin.settings.apollosUrl}/settings#clients`,
			attr: { target: "_blank" },
		});

		new Setting(containerEl)
			.setName("Apollos URL")
			.setDesc("The URL of the Apollos backend.")
			.addText((text) =>
				text
					.setValue(`${this.plugin.settings.apollosUrl}`)
					.onChange(async (value) => {
						this.plugin.settings.apollosUrl = value
							.trim()
							.replace(/\/$/, "");
						({
							connectedToBackend:
		...
		new Setting(containerEl)
			...
			.setDesc(
				"The number of results to show in search and use for chat.",
			)
			.addSlider((slider) =>
				slider
					.setLimits(1, 30, 1)
		...
		new Setting(containerEl)
			.setName("Auto Sync")
			.setDesc("Automatically index your vault with Apollos.")
			.addToggle((toggle) =>
				toggle
		...
		new Setting(containerEl)
			.setName("Sync Notes")
			.setDesc("Index Markdown files in your vault with Apollos.")
			.addToggle((toggle) =>
				toggle
		...
		new Setting(containerEl)
			.setName("Sync Images")
			.setDesc("Index images in your vault with Apollos.")
			.addToggle((toggle) =>
				toggle
		...
		new Setting(containerEl)
			.setName("Sync PDFs")
			.setDesc("Index PDF files in your vault with Apollos.")
			.addToggle((toggle) =>
				toggle
		...
		const includeFoldersContainer = containerEl.createDiv(
			...
			.addButton((button) =>
				button.setButtonText("Add Folder").onClick(() => {
					const modal = new FolderSuggestModal(
						...
						async (folder: string) => {
							if (
								!this.plugin.settings.syncFolders.includes(
									folder,
								)
							) {
								this.plugin.settings.syncFolders.push(folder);
								await this.plugin.saveSettings();
								this.updateIncludeFolderList(
									includeFolderListEl,
								);
								this.refreshStorageDisplay();
							}
		...
		const includeFolderListEl =
			includeFoldersContainer.createDiv("folder-list");
		...
		const excludeFolderListEl =
			excludeFoldersContainer.createDiv("folder-list");
		this.updateExcludeFolderList(excludeFolderListEl);

		let indexVaultSetting = new Setting(containerEl);
		indexVaultSetting
			.setName("Force Sync")
			.setDesc("Manually force Apollos to re-index your Obsidian Vault.")
			.addButton((button) =>
				button
					...
					.onClick(async () => {
						...
						const progress_indicator = window.setInterval(() => {
							if (button.buttonEl.innerText === "Updating 🌑") {
								...
								button.buttonEl.innerText === "Updating 🌘"
							) {
								button.setButtonText("Updating 🌗");
							} else if (
								button.buttonEl.innerText === "Updating 🌗"
						...
						const syncProgressEl = document.getElementById(
							"apollos-sync-progress",
						) as HTMLProgressElement | null;
						const syncProgressText = document.getElementById(
							"apollos-sync-progress-text",
						) as HTMLElement | null;
						...
						try {
							...
							button.setCta();
							indexVaultSetting =
								indexVaultSetting.setDisabled(false);
						}
					}),
			);
		...
		const storageSetting = new Setting(containerEl)
			...
			.setDesc(
				"Estimated storage usage based on files configured for sync. This is a client-side estimation.",
			)
			.then(() => {});
	...
	private connectStatusIcon() {
		if (
			...
			return "🟢";
		else if (this.plugin.settings.connectedToBackend) return "🟡";
		else return "🔴";
	}
	...
	private async refreshStorageDisplay() {
		...
		try {
			...
			const metrics = await calculateVaultSyncMetrics(
				this.app.vault,
				this.plugin.settings,
			);
			...
			console.error("Apollos: Failed to update storage display", err);
	...
	private async refreshModelsAndServerPreference() {
		...
		if (this.plugin.settings.connectedToBackend) {
			...
			if (
				serverConfig &&
				serverConfig.selected_chat_model_config !== undefined &&
				serverConfig.selected_chat_model_config !== null
			) {
				const serverModelIdStr =
					serverConfig.selected_chat_model_config.toString();
				// Ensure the server's selected model is actually in the available list
				if (
					this.plugin.settings.availableChatModels.some(
						(m) => m.id === serverModelIdStr,
					)
				) {
					...
					// In this case, we might fall back to null (Apollos Default)
					console.warn(
						`Apollos: Server's selected model ID ${serverModelIdStr} not in available models. Falling back to default.`,
					);
	...
	private renderChatModelDropdown() {
		if (!this.chatModelSetting) {
			this.chatModelSetting = new Setting(this.containerEl).setName(
				"Chat Model",
			);
		} else {
		...
		if (!this.plugin.settings.connectedToBackend) {
			modelSetting.setDesc(
				"Connect to Apollos to load and set chat model options.",
			);
			modelSetting.addText((text) =>
				text.setValue("Not connected").setDisabled(true),
			);
			return;
		}

		if (
			this.plugin.settings.availableChatModels.length === 0 &&
			this.plugin.settings.connectedToBackend
		) {
			modelSetting.setDesc(
				"Fetching models or no models available. Check Apollos connection or try refreshing.",
			);
		...
		modelSetting.addDropdown((dropdown) => {
			...
			dropdown
				...
				.onChange(async (value) => {
					...
					const success = await updateServerChatModel(
						value,
						this.plugin.settings,
					);
					if (success) {
						...
						dropdown.setValue(
							this.plugin.settings.selectedChatModelId || "",
						);
					}
	...
	private updateIncludeFolderList(containerEl: HTMLElement) {
		this.updateFolderList(
			...
			async (folder) => {
				this.plugin.settings.syncFolders =
					this.plugin.settings.syncFolders.filter(
						(f) => f !== folder,
					);
				await this.plugin.saveSettings();
