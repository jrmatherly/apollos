...
export interface ApolloslosSetting {
    resultsCount: number;
    apollosUrl: string;
...
export const DEFAULT_SETTINGS: ApolloslosSetting = {
    resultsCount: 15,
    apollosUrl: 'https://app.apollos.dev',
    apollosApiKey: '',
...
export class ApolloslosSettingTab extends PluginSettingTab {
    plugin: Apolloslos;
    ...
    constructor(app: App, plugin: Apolloslos) {
        super(app, plugin);
        this.plugin = plugin;
    }
    ...
    display(): void {
        ...
        const connectHeaderContentEl = connectHeaderEl.createSpan({ cls: 'apollos-connect-settings-header' });
        const connectTitleEl = connectHeaderContentEl.createSpan({ text: 'Connect' });
        const backendStatusEl = connectTitleEl.createSpan({ text: this.connectStatusIcon(), cls: 'apollos-connect-settings-header-status' });
        ...
        const apiKeySetting = new Setting(containerEl)
            ...
            .addText(text => text
                ...
                .onChange(async (value) => {
                    ...
                    } = await canConnectToBackend(this.plugin.settings.apollosUrl, this.plugin.settings.apollosApiKey));
        ...
        apiKeySetting.descEl.createEl('span', {
            text: 'Connect your Apolloslos Cloud account. ',
        });
        ...
        new Setting(containerEl)
            .setName('Apolloslos URL')
            .setDesc('The URL of the Apolloslos backend.')
            .addText(text => text
                .setValue(`${this.plugin.settings.apollosUrl}`)
                .onChange(async (value) => {
                    this.plugin.settings.apollosUrl = value.trim().replace(/\/$/, '');
                    ({
                        connectedToBackend: this.plugin.settings.connectedToBackend,
                        userInfo: this.plugin.settings.userInfo,
                        statusMessage: backendStatusMessage,
        ...
        new Setting(containerEl)
            .setName('Auto Sync')
            .setDesc('Automatically index your vault with Apolloslos.')
            .addToggle(toggle => toggle
        ...
        new Setting(containerEl)
            .setName('Sync Notes')
            .setDesc('Index Markdown files in your vault with Apolloslos.')
            .addToggle(toggle => toggle
        ...
        new Setting(containerEl)
            .setName('Sync Images')
            .setDesc('Index images in your vault with Apolloslos.')
            .addToggle(toggle => toggle
        ...
        new Setting(containerEl)
            .setName('Sync PDFs')
            .setDesc('Index PDF files in your vault with Apolloslos.')
            .addToggle(toggle => toggle
        ...
        indexVaultSetting
            .setName('Force Sync')
            .setDesc('Manually force Apolloslos to re-index your Obsidian Vault.')
            .addButton(button => button
                ...
                .onClick(async () => {
                    ...
                    const syncProgressEl = document.getElementById('apollos-sync-progress') as HTMLProgressElement | null;
                    const syncProgressText = document.getElementById('apollos-sync-progress-text') as HTMLElement | null;
                    ...
                    const onProgress = (progress: { processed: number, total: number }) => {
                        const el = document.getElementById('apollos-sync-progress') as HTMLProgressElement | null;
                        const txt = document.getElementById('apollos-sync-progress-text') as HTMLElement | null;
                        if (!el || !txt) return;
    ...
    private async refreshStorageDisplay() {
        ...
        try {
            ...
            console.error('Apolloslos: Failed to update storage display', err);
        ...
        if (this.plugin.settings.connectedToBackend) {
            ...
            if (serverConfig && serverConfig.selected_chat_model_config !== undefined && serverConfig.selected_chat_model_config !== null) {
                ...
                if (this.plugin.settings.availableChatModels.some(m => m.id === serverModelIdStr)) {
                    ...
                    // In this case, we might fall back to null (Apolloslos Default)
                    console.warn(`Apolloslos: Server's selected model ID ${serverModelIdStr} not in available models. Falling back to default.`);
    ...
    private renderChatModelDropdown() {
        ...
        if (!this.plugin.settings.connectedToBackend) {
            modelSetting.setDesc('Connect to Apolloslos to load and set chat model options.');
        ...
        if (this.plugin.settings.availableChatModels.length === 0 && this.plugin.settings.connectedToBackend) {
            modelSetting.setDesc('Fetching models or no models available. Check Apolloslos connection or try refreshing.');
