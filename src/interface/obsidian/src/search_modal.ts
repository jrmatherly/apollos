...
export class ApollosSearchModal extends SuggestModal<SearchResult> {
	...
	constructor(
		...
		this.scope.register(["Mod"], "Enter", async () => {
			...
			this.rerank = true;
			// Trigger input event to get and render (reranked) results from apollos backend
		...
		this.setPlaceholder("Search with Apollos...");
	}
	...
	private isFileInVault(filePath: string): boolean {
		...
		return this.app.vault
			.getFiles()
			.some((file) => file.path === normalizedPath);
	}
	...
	async getSuggestions(query: string): Promise<SearchResult[]> {
		...
		try {
			...
			let searchUrl = `${this.setting.apollosUrl}/api/search?q=${encodedQuery}&n=${this.setting.resultsCount}&r=${this.rerank}&client=obsidian`;
			let headers = {
			...
			let results = data
				.filter(
					(result: any) =>
						!this.find_similar_notes ||
						!result.additional.file.endsWith(
				...
				.sort(
					(
						a: SearchResult & { inVault: boolean },
						b: SearchResult & { inVault: boolean },
					) => {
	...
	async onOpen() {
		if (this.find_similar_notes) {
			...
			if (file && file.extension === "md") {
				...
				this.inputEl.value = await this.app.vault
					.read(file)
					.then((file_str) => file_str.slice(0, 42110));
				// Trigger search to get and render similar notes from apollos backend
				this.inputEl.dispatchEvent(new Event("input"));
				this.rerank = false;
			} else {
				this.resultContainerEl.setText(
					"Cannot find similar notes for non-markdown files",
				);
			}
		}
	}
	...
	async renderSuggestion(
		result: SearchResult & { inVault: boolean },
		el: HTMLElement,
	) {
		...
		let result_el = el.createEl("div", { cls: "apollos-result-entry" });

		let resultToRender = "";
		let fileExtension = filename?.split(".").pop() ?? "";
		if (
			supportedImageFilesTypes.includes(fileExtension) &&
			filename &&
			result.inVault
		) {
