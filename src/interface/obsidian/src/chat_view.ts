...
export class ApollosChatView extends ApollosPaneView {
	result: string;
	...
	constructor(leaf: WorkspaceLeaf, plugin: Apollos) {
		...
		this.scope.register(["Ctrl", "Alt"], "n", (_) =>
			this.createNewConversation(this.currentAgent),
		);
		this.scope.register(
			["Ctrl", "Alt"],
			"o",
		...
		this.scope.register(["Ctrl", "Alt"], "v", (_) =>
			this.speechToText(
				this.voiceChatActive
					? new KeyboardEvent("keyup")
					: new KeyboardEvent("keydown"),
			),
		);
		this.scope.register(["Ctrl"], "f", (_) =>
			new ApollosSearchModal(this.app, this.setting).open(),
		);
	...
	async chat(isVoice: boolean = false) {
		...
		let input_el = <HTMLTextAreaElement>(
			this.contentEl.getElementsByClassName("apollos-chat-input")[0]
		);
		...
		if (user_message) {
			...
			const selectedMode = this.chatModes.find((mode) =>
				this.contentEl.querySelector(
					`#apollos-mode-${mode.value}:checked`,
				),
			);
	...
	async onOpen() {
		...
		let defaultDomains = `'self' ${this.setting.apollosUrl} https://*.obsidian.md https://app.apollosai.dev https://assets.apollosai.dev`;
		...
		// WARNING: CSP DISABLED for now as it breaks other Obsidian plugins. Enable when can scope CSP to only Apollos plugin.
		// CSP meta tag for the Apollos Chat modal
		...
		const headerAgentSelect = this.contentEl.querySelector(
			".apollos-header-agent-select",
		...
		let chatBodyEl = contentEl.createDiv({
			attr: { id: "apollos-chat-body", class: "apollos-chat-body" },
		});
		...
		// Get chat history from Apollos backend and set chat input state
		let getChatHistorySucessfully = await this.getChatHistory(chatBodyEl);

		let placeholderText: string = getChatHistorySucessfully
			? this.startingMessage
			: "Configure Apollos to enable chat";
		...
		if (!this.keyPressTimeout) {
			this.keyPressTimeout = setTimeout(async () => {
				...
				if (this.sendMessageTimeout) {
					...
					let sendImg = <SVGElement>(
						sendButton.getElementsByClassName(
	...
	textToSpeech(message: string, event: MouseEvent | null = null): void {
		...
		if (event === null) {
			...
			let speechButtons =
				document.getElementsByClassName("speech-button");
			speechButton = speechButtons[
				speechButtons.length - 1
	...
	formatHTMLMessage(message: string, raw = false, willReplace = true) {
		...
		chatMessageBodyTextEl.innerHTML = this.markdownTextToSanitizedHtml(
			message,
			this,
		);
	...
	markdownTextToSanitizedHtml(
		...
		virtualChatMessageBodyTextEl.innerHTML =
			virtualChatMessageBodyTextEl.innerHTML.replace(
	...
	private processTrainOfThought(message: string): {
		content: string;
		header: string;
	} {
	...
	renderMessageWithReferences(
		...
		if (
			...
			let imageMarkdown = this.generateImageMarkdown(
				message,
				intentType ?? "",
				inferredQueries,
				conversationId,
	...
	generateImageMarkdown(
		...
		if (intentType === "text-to-image") {
			...
			const domain = this.setting.apollosUrl.endsWith("/")
				? this.setting.apollosUrl
				: `${this.setting.apollosUrl}/`;
	...
	createApollosResponseDiv(dt?: Date): HTMLDivElement {
		let messageTime = this.formatDate(dt ?? new Date());

		// Append message to conversation history HTML element.
		// The chat logs should display above the message input box to follow standard UI semantics
		let chatBodyEl =
			this.contentEl.getElementsByClassName("apollos-chat-body")[0];
		let chatMessageEl = chatBodyEl.createDiv({
			attr: {
	...
	async renderIncrementalMessage(
		...
		// Sanitize the markdown to render
		let sanitizedResponse = DOMPurify.sanitize(
		...
		tempElement.innerHTML = this.markdownTextToSanitizedHtml(
			transformedResponse,
			this,
		);
	...
	renderActionButtons(
		...
		if (
			!isSystemMessage &&
			chatMessageBodyTextEl.closest(".apollos-chat-message.you")
		) {
			...
			editButton.addEventListener("click", async () => {
				const messageEl = chatMessageBodyTextEl.closest(
					".apollos-chat-message",
				);
				if (messageEl) {
		...
		if (this.setting.userInfo?.is_active) {
			...
			speechButton.addEventListener("click", (event) =>
				this.textToSpeech(message, event),
			);
	...
	async createNewConversation(agentSlug?: string | null) {
		...
		try {
			...
			if (response.ok) {
				...
				const agentSelect = this.contentEl.querySelector(
					".apollos-header-agent-select",
				) as HTMLSelectElement;
				if (agentSelect) {
	...
	async toggleChatSessions(forceShow: boolean = false): Promise<boolean> {
		...
		try {
			let response = await fetch(chatSessionsUrl, {
				method: "GET",
				headers: headers,
			});
	...
	addConversationMenu(
			...
			editConversationTitleSaveButtonEl.addEventListener(
				...
				(event) => {
					...
					if (newTitle != null) {
						...
						fetch(`${this.setting.apollosUrl}${editURL}`, {
							method: "PATCH",
							headers,
						})
							.then((response) =>
							...
							.then((data) => {
								conversationSessionTitleEl.textContent =
									newTitle;
							})
							.catch((err) => {
								return;
						...
						conversationSessionTitleEl.addEventListener(
							"click",
							() => {
								chatBodyEl.innerHTML = "";
								chatBodyEl.dataset.conversationId =
		...
		deleteConversationButtonEl.addEventListener("click", () => {
			...
			fetch(`${this.setting.apollosUrl}${deleteURL}`, {
				method: "DELETE",
				headers,
			})
				.then((response) =>
	...
	async getChatHistory(chatBodyEl: HTMLElement): Promise<boolean> {
		// Get chat history from Apollos backend
		...
		try {
			...
			if (responseJson.detail) {
				...
				chatBodyEl.dataset.conversationId =
					responseJson.response.conversation_id;
				chatBodyEl.dataset.conversationTitle =
					responseJson.response.slug || `New conversation 🌱`;

				// Update current agent from conversation history
				if (responseJson.response.agent?.slug) {
					console.debug(
						"Found agent in conversation history:",
						responseJson.response.agent,
					);
					...
					if (agentSelect && this.currentAgent) {
						agentSelect.value = this.currentAgent;
						console.log(
							"Updated agent selector to:",
							this.currentAgent,
						);
					}
				}

				let chatLogs = responseJson.response?.conversation_id
				...
				chatLogs.forEach((chatLog: any) => {
					...
					if (chatLog.by === "apollos") {
						chatLog.message = this.transformEditBlocks(
							chatLog.message,
						);
					}
	...
	async processMessageChunk(rawChunk: string): Promise<void> {
		...
		if (chunk.type === "start_llm_response") {
			...
			const imageData = this.handleImageResponse(
				generatedAssets,
				this.chatMessageState.rawResponse,
			);
			...
			if (this.fileAccessMode === "write") {
				const editBlocks = this.parseEditBlocks(
					this.chatMessageState.rawResponse,
				);
				...
				if (editBlocks.length > 0) {
					...
					if (firstBlock.hasError) {
						...
						if (this.editRetryCount < this.maxEditRetries) {
							...
							console.warn(
								"[Apollos] Max edit retries reached. Aborting further retries.",
							);
						}
					} else {
			...
			if (
				this.chatMessageState.isVoice &&
				this.setting.userInfo?.is_active &&
				this.setting.autoVoiceResponse
			)
			...
			this.chatMessageState.references = {
				notes: chunk.data.context,
				online: chunk.data.onlineContext,
			};
			...
			if (typeof chunkData === "object" && chunkData !== null) {
				...
				try {
					...
					this.handleStreamResponse(
						this.chatMessageState.newResponseTextEl,
						this.chatMessageState.rawResponse +
							this.chatMessageState.generatedAssets,
						this.chatMessageState.loadingEllipsis,
					);
				}
			} else {
	...
	handleJsonResponse(jsonData: any): void {
		if (
			...
			jsonData.detail ||
			jsonData.images ||
			jsonData.mermaidjsDiagram
		) {
			this.chatMessageState.rawResponse = this.handleImageResponse(
				jsonData,
				this.chatMessageState.rawResponse,
			);
		...
		if (this.chatMessageState.newResponseTextEl) {
			...
			this.chatMessageState.newResponseTextEl.appendChild(
				this.formatHTMLMessage(this.chatMessageState.rawResponse),
			);
		}
	}
	...
	async getChatResponse(
		...
		if (!conversationId) {
			try {
				...
				const response = await fetch(
					`${this.setting.apollosUrl}/api/chat/sessions`,
					{
						method: "POST",
						headers: {
							Authorization: `Bearer ${this.setting.apollosApiKey}`,
		...
		// Get chat response from Apollos backend
	...
	async clearConversationHistory() {
		...
		try {
			...
			if (result.status !== "ok") {
				...
				let statusMsg = getChatHistoryStatus
					? result.message
	...
	async speechToText(event: MouseEvent | TouchEvent | KeyboardEvent) {
		event.preventDefault();
		const transcribeButton = <HTMLButtonElement>(
			this.contentEl.getElementsByClassName("apollos-transcribe")[0]
		);
		const chatInput = <HTMLTextAreaElement>(
			this.contentEl.getElementsByClassName("apollos-chat-input")[0]
		);
		const sendButton = <HTMLButtonElement>(
			this.contentEl.getElementsByClassName("apollos-chat-send")[0]
		);

		const generateRequestBody = async (
			audioBlob: Blob,
			boundary_string: string,
		) => {
		...
		const sendToServer = async (audioBlob: Blob) => {
			...
			// Parse response from Apollos backend
			...
			if (response.status === 200) {
				...
				noSpeech = noSpeechText.includes(
					response.json.text.trimStart(),
				);
				if (!noSpeech)
			...
			let stopSendButtonImg = <SVGElement>(
				sendButton.getElementsByClassName("lucide-stop-circle")[0]
			);
			...
			stopSendButtonImg.getElementsByTagName(
				"circle",
			)[0].style.animation = "countdown 3s linear 1 forwards";
			stopSendButtonImg.getElementsByTagName("circle")[0].style.color =
	...
	handleImageResponse(imageJson: any, rawResponse: string) {
		if (imageJson.image) {
			const inferredQuery =
				imageJson.inferredQueries?.[0] ?? "generated image";
	...
	private highlightVisibleOption(visibleOptions: HTMLElement[]) {
		...
		if (
			...
			selectedOption.classList.add(
				"apollos-mode-dropdown-option-selected",
			);
	...
	async deleteMessage(
		...
		if (!skipPaired) {
			const messages = Array.from(
				document.getElementsByClassName("apollos-chat-message"),
			);
			...
			// If we're deleting a apollos message, also delete the preceding user message (if any)
				...
				messageContainer.classList.contains("apollos") &&
				currentIndex > 0
			) {
				pairedMessageContainer = messages[currentIndex - 1];
			}
		...
		setTimeout(async () => {
			...
			if (!skipBackend && turnId) {
				...
				try {
					// Delete from backend
					const response = await fetch(
						`${this.setting.apollosUrl}/api/chat/conversation/message`,
						{
							method: "DELETE",
							headers: {
								"Content-Type": "application/json",
	...
	async fetchAgents() {
		try {
			const response = await fetch(
				`${this.setting.apollosUrl}/api/agents`,
				{
					headers: {
						Authorization: `Bearer ${this.setting.apollosApiKey}`,
					},
	...
	private async applyEditBlocks(editBlocks: EditBlock[]) {
		...
		const { editResults, fileBackups } =
			await this.fileInteractions.applyEditBlocks(
				editBlocks,
				(blockToRetry) => {
					if (this.editRetryCount < this.maxEditRetries) {
		...
		if (lastMessage) {
			const buttonsContainer = lastMessage.createDiv({
				cls: "edit-confirmation-buttons",
			});

			// Create a dedicated container for status summary to ensure proper separation
			const statusContainer = buttonsContainer.createDiv({
				cls: "edit-status-container",
			});

			// Add status summary as a separate element with its own styling
			const statusSummary = statusContainer.createDiv({
				cls: "edit-status-summary",
			});
			...
			if (editResults.some((r) => !r.success)) {
				const errorDetails = editResults
					...
					.map((r) => {
						...
						if (
							r.error &&
							r.error.includes("Other edits in the group failed")
						) {
	...
	private setupConfirmationButtons(
		...
		applyButton.addEventListener("click", async () => {
			try {
				...
				const successMessage = lastMessage.createDiv({
					cls: "edit-status-message success",
				});
				successMessage.textContent = "Changes applied successfully";
				setTimeout(() => successMessage.remove(), 3000);
			} catch (error) {
				console.error("Error applying changes:", error);
				const errorMessage = lastMessage.createDiv({
					cls: "edit-status-message error",
				});
	...
	public async applyPendingEdits() {
		...
		const buttonsContainer = lastMessage.querySelector(
		...
		const applyButton =
			buttonsContainer.querySelector(".edit-apply-button");
	...
	private async handleEditRetry(errorBlock: EditBlock) {
		...
		if (errorBlock.error?.type === "missing_field") {
			...
			errorDetails +=
				"Please check the syntax and provide a valid JSON edit block.\n";
		} else {
		...
		const retryContainer = chatBodyEl.createDiv({
			cls: "apollos-retry-container",
		});

		// Create the retry badge inside the container
		const retryBadge = retryContainer.createDiv({
			cls: "apollos-retry-badge",
		});
