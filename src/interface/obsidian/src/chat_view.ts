...
import { ApolloslosView, createCopyParentText, getLinkToEntry, pasteTextAtCursor } from 'src/utils';
import { ApolloslosSearchModal } from 'src/search_modal';
import Apolloslos from 'src/main';
...
export class ApolloslosChatView extenApollospollosPaneView {
    result: string;
    ...
    constructor(leaf: WorkspaceLeaf, plugin: Apolloslos) {
        super(leaf, plugin);
        ...
        this.scope.register(["Ctrl"], 'f', (_) => new ApolloslosSearchModal(this.app, this.setting).open());
        this.scope.register(["Ctrl"], 'r', (_) => { this.activateView(ApolloslosView.SIMILAR); });
    }

    getViewType(): string {
        return ApolloslosView.CHAT;
    }

    getDisplayText(): string {
        return "Apolloslos Chat";
    }
    ...
    async chat(isVoice: boolean = false) {
        ...
        let input_el = <HTMLTextAreaElement>this.contentEl.getElementsByClassName("apollos-chat-input")[0];
        ...
        if (user_message) {
            ...
            const selectedMode = this.chatModes.find(mode =>
                this.contentEl.querySelector(`#apollos-mode-${mode.value}:checked`)
            );
    ...
    async onOpen() {
        ...
        let defaultDomains = `'self' ${this.setting.apollosUrl} https://*.obsidian.md https://app.apollos.dev https://assets.apollos.dev`;
        const defaultSrc = `default-src ${defaultDomains};`;
        const scriptSrc = `script-src ${defaultDomains} 'unsafe-inline';`;
        const connectSrc = `connect-src ${this.setting.apollosUrl} wss://*.obsidian.md/ https://ipapi.co/json;`;
        ...
        // WARNING: CSP DISABLED for now as it breaks other Obsidian plugins. Enable when can scope CSP to only Apolloslos plugin.
        // CSP meta tag for the Apolloslos Chat modal
        ...
        const headerAgentSelect = this.contentEl.querySelector('.apollos-header-agent-select') as HTMLSelectElement;
        if (headerAgentSelect && this.agents.length > 0) {
            ...
            this.agents.forEach(agent => {
                if (agent.slug === 'apollos') return; // Skip the default agent
        ...
        contentEl.addClass("apollos-chat");

        // Create the chat body
        let chatBodyEl = contentEl.createDiv({ attr: { id: "apollos-chat-body", class: "apollos-chat-body" } });
        // Add chat input field
        let inputRow = contentEl.createDiv("apollos-input-row");

        let chatSessions = inputRow.createEl("button", {
            text: "Chat Sessions",
            attr: {
                class: "apollos-input-row-button clickable-icon",
        ...
        let chatInput = inputRow.createEl("textarea", {
            attr: {
                id: "apollos-chat-input",
                autofocus: "autofocus",
                class: "apollos-chat-input option",
            },
        })
        ...
        let transcribe = inputRow.createEl("button", {
            text: "Transcribe",
            attr: {
                id: "apollos-transcribe",
                class: "apollos-transcribe apollos-input-row-button clickable-icon ",
        ...
        let send = inputRow.createEl("button", {
            text: "Send",
            attr: {
                id: "apollos-chat-send",
                class: "apollos-chat-send apollos-input-row-button clickable-icon",
            },
        })
        ...
        // Get chat history from Apolloslos backend and set chat input state
        let getChatHistorySucessfully = await this.getChatHistory(chatBodyEl);

        let placeholderText: string = getChatHistorySucessfully ? this.startingMessage : "Configure Apolloslos to enable chat";
        ...
        requestAnimationFrame(() => {
            ...
            requestAnimationFrame(() => {
                ...
                const chatInput = <HTMLTextAreaElement>this.contentEl.getElementsByClassName("apollos-chat-input")[0];
                chatInput?.focus();
            });
        });
    }
        ...
        if (!this.keyPressTimeout) {
            this.keyPressTimeout = setTimeout(async () => {
                ...
                if (this.sendMessageTimeout) {
                    ...
                    const sendButton = <HTMLButtonElement>this.contentEl.getElementsByClassName("apollos-chat-send")[0]
    ...
    textToSpeech(message: string, event: MouseEvent | null = null): void {
        ...
        let textToSpeechApi = `${this.setting.apollosUrl}/api/chat/speech?text=${encodeURIComponent(message)}`;
        fetch(textToSpeechApi, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                "Authorization": `Bearer ${this.setting.apollosApiKey}`,
            },
        })
    ...
    markdownTextToSanitizedHtml(markdownText: string, component: ItemView): string {
        ...
        virtualChatMessageBodyTextEl.innerHTML = virtualChatMessageBodyTextEl.innerHTML.replace(
            /<img(?:(?!src=["'](app:|data:|https:\/\/generated\.apollos\.dev)).)*?>/gis,
            ''
        );
    ...
    renderMessageWithReferences(
        ...
        let chatMessageBodyEl = chatMessageEl.getElementsByClassName("apollos-chat-message-text")[0];
    ...
    generateImageMarkdown(message: string, intentType: string, inferredQueries?: string[], conversationId?: string, images?: string[], excalidrawDiagram?: string, mermaidjsDiagram?: string): string {
        ...
        if (intentType === "text-to-image") {
            ...
            const domain = this.setting.apollosUrl.endsWith("/") ? this.setting.apollosUrl : `${this.setting.apollosUrl}/`;
    ...
    renderMessage({ chatBodyEl, message, sender, dt, turnId, raw = false, willReplace = true, isSystemMessage = false }: RenderMessageOptions): Element {
        ...
        let chatMessageEl = chatBodyEl.createDiv({
            attr: {
                ...
                class: `apollos-chat-message ${sender}`,
                ...(turnId && { "data-turnid": turnId })
            },
        })
        let chatMessageBodyEl = chatMessageEl.createDiv();
        chatMessageBodyEl.addClasses(["apollos-chat-message-text", sender]);
    ...
    createApolloslosResponseDiv(dt?: Date): HTMLDivElement {
        let messageTime = this.formatDate(dt ?? new Date());

        // Append message to conversation history HTML element.
        // The chat logs should display above the message input box to follow standard UI semantics
        let chatBodyEl = this.contentEl.getElementsByClassName("apollos-chat-body")[0];
        let chatMessageEl = chatBodyEl.createDiv({
            attr: {
                "data-meta": messageTime,
                class: `apollos-chat-message apollos`
            },
        })
    ...
    renderActionButtons(message: string, chatMessageBodyTextEl: HTMLElement, isSystemMessage: boolean = false) {
        ...
        let editButton = null;
        if (!isSystemMessage && chatMessageBodyTextEl.closest('.apollos-chat-message.you')) {
            ...
            editButton.addEventListener('click', async () => {
                const messageEl = chatMessageBodyTextEl.closest('.apollos-chat-message');
                if (messageEl) {
                    // Get all messages up to this one
                    const allMessages = Array.from(this.contentEl.getElementsByClassName('apollos-chat-message'));
                    ...
                    const chatInput = this.contentEl.querySelector('.apollos-chat-input') as HTMLTextAreaElement;
                    if (chatInput) {
    ...
    async createNewConversation(agentSlug?: string | null) {
        let chatBodyEl = this.contentEl.getElementsByClassName("apollos-chat-body")[0] as HTMLElement;
        ...
        try {
            ...
            let endpoint = `${this.setting.apollosUrl}/api/chat/sessions`;
            ...
            if (response.ok) {
                ...
                const agentSelect = this.contentEl.querySelector('.apollos-header-agent-select') as HTMLSelectElement;
                if (agentSelect) {
        ...
        this.renderMessage({ chatBodyEl, message: "Hey, what's up?", sender: "apollos", isSystemMessage: true });
    }
    ...
    async toggleChatSessions(forceShow: boolean = false): Promise<boolean> {
        ...
        const chatSessionsUrl = `${this.setting.apollosUrl}/api/chat/sessions?client=obsidian`;
        const headers = { 'Authorization': `Bearer ${this.setting.apollosApiKey}` };
        try {
    ...
    addConversationMenu(
        ...
        editConversationTitleButtonEl.addEventListener('click', (event) => {
            ...
            editConversationTitleSaveButtonEl.addEventListener('click', (event) => {
                ...
                if (newTitle != null) {
                    ...
                    fetch(`${this.setting.apollosUrl}${editURL}`, { method: "PATCH", headers })
        ...
        deleteConversationButtonEl.addEventListener('click', () => {
            ...
            fetch(`${this.setting.apollosUrl}${deleteURL}`, { method: "DELETE", headers })
                .then(response => response.ok ? response.json() : Promise.reject(response))
                .then(data => {
                    chatBodyEl.innerHTML = "";
                    chatBodyEl.dataset.conversationId = "";
    ...
    async getChatHistory(chatBodyEl: HTMLElement): Promise<boolean> {
        // Get chat history from Apolloslos backend
        let chatUrl = `${this.setting.apollosUrl}/api/chat/history?client=obsidian`;
        ...
        try {
            let response = await fetch(chatUrl, {
                method: "GET",
                headers: { "Authorization": `Bearer ${this.setting.apollosApiKey}` },
            });
            ...
            if (responseJson.detail) {
                ...
                chatLogs.forEach((chatLog: any) => {
                    ...
                    // Transform apollos-edit blocks into accordions
                    if (chatLog.by === "apollos") {
            ...
            let errorMsg = "Unable to get response from Apolloslos server ❤️‍🩹. Ensure server is running or contact developers for help at [team@apollos.dev](mailto:team@apollos.dev) or in [Discord](https://discord.gg/BDgyabRM6e)";
            this.renderMessage({
                chatBodyEl,
                message: errorMsg,
                sender: "apollos",
                ...
                if (editBlocks.length > 0) {
                    ...
                    if (firstBlock.hasError) {
                        ...
                        if (this.editRetryCount < this.maxEditRetries) {
                            ...
                            console.warn('[Apolloslos] Max edit retries reached. Aborting further retries.');
                        }
                    } else {
    ...
    async getChatResponse(query: string | undefined | null, displayQuery: string | undefined | null, isVoice: boolean = false, displayUserMessage: boolean = true): Promise<void> {
        ...
        if (!conversationId) {
            try {
                ...
                const response = await fetch(`${this.setting.apollosUrl}/api/chat/sessions`, {
                    method: "POST",
                    headers: {
                        "Authorization": `Bearer ${this.setting.apollosApiKey}`,
                        "Content-Type": "application/json"
        ...
        // Get chat response from Apolloslos backend
        const chatUrl = `${this.setting.apollosUrl}/api/chat?client=obsidian`;
        const body = {
            q: finalQuery,
        ...
        let newResponseEl = this.createApolloslosResponseDiv();
        let newResponseTextEl = newResponseEl.createDiv();
        newResponseTextEl.classList.add("apollos-chat-message-text", "apollos");

        // Temporary status message to indicate that Apolloslos is thinking
        ...
        try {
            ...
            console.error(`Apolloslos chat response failed with\n${err}`);
            let errorMsg = "Sorry, unable to get response from Apolloslos backend ❤️‍🩹. Retry or contact developers for help at <a href=mailto:'team@apollos.dev'>team@apollos.dev</a> or <a href='https://discord.gg/BDgyabRM6e'>on Discord</a>";
    ...
    flashStatusInChatInput(message: string) {
        ...
        let chatInput = <HTMLTextAreaElement>this.contentEl.getElementsByClassName("apollos-chat-input")[0];
    ...
    async clearConversationHistory() {
        let chatBody = this.contentEl.getElementsByClassName("apollos-chat-body")[0] as HTMLElement;

        let response = await request({
            url: `${this.setting.apollosUrl}/api/chat/history?client=obsidian`,
            method: "DELETE",
            headers: { "Authorization": `Bearer ${this.setting.apollosApiKey}` },
        })
        try {
    ...
    async speechToText(event: MouseEvent | TouchEvent | KeyboardEvent) {
        event.preventDefault();
        const transcribeButton = <HTMLButtonElement>this.contentEl.getElementsByClassName("apollos-transcribe")[0];
        const chatInput = <HTMLTextAreaElement>this.contentEl.getElementsByClassName("apollos-chat-input")[0];
        const sendButton = <HTMLButtonElement>this.contentEl.getElementsByClassName("apollos-chat-send")[0]
        ...
        const sendToServer = async (audioBlob: Blob) => {
            ...
            const response = await requestUrl({
                url: `${this.setting.apollosUrl}/api/transcribe?client=obsidian`,
                method: 'POST',
                headers: { "Authorization": `Bearer ${this.setting.apollosApiKey}` },
            ...
            // Parse response from Apolloslos backend
    ...
    cancelSendMessage() {
        ...
        let sendButton = <HTMLButtonElement>this.contentEl.getElementsByClassName("apollos-chat-send")[0];
        setIcon(sendButton, "arrow-up-circle");
        let sendImg = <SVGElement>sendButton.getElementsByClassName("lucide-arrow-up-circle")[0]
        sendImg.addEventListener('click', async (_) => { await this.chat() });
    };
    ...
    incrementalChat(event: KeyboardEvent) {
        ...
        if (this.modeDropdown && this.modeDropdown.style.display !== "none" && event.key === "Enter") {
            ...
            const options = this.modeDropdown.querySelectorAll<HTMLElement>(".apollos-mode-dropdown-option");
    ...
    scrollChatToBottom() {
        const chat_body_el = this.contentEl.getElementsByClassName("apollos-chat-body")[0];
    ...
    handleStreamResponse(newResponseElement: HTMLElement | null, rawResponse: string, loadingEllipsis: HTMLElement | null, replace = true) {
        ...
        messageEl.classList.add('apollos-message-new-content');
        newResponseElement.appendChild(messageEl);

        // Remove the animation class after the animation completes
        setTimeout(() => {
            newResponseElement.classList.remove('apollos-message-new-content');
        }, 300);
    }
    ...
    finalizeChatBodyResponse(references: object, newResponseElement: HTMLElement | null, turnId: string) {
        ...
        this.scrollChatToBottom();
        let chatInput = this.contentEl.getElementsByClassName("apollos-chat-input")[0];
    ...
    private highlightVisibleOption(visibleOptions: HTMLElement[]) {
        const allOptions = this.modeDropdown?.querySelectorAll<HTMLElement>(".apollos-mode-dropdown-option");
        if (!allOptions) return;

        // Clear highlighting on all options first
        allOptions.forEach(option => {
            option.classList.remove("apollos-mode-dropdown-option-selected");
        });

        // Add highlighting to the selected visible option
        if (this.selectedOptionIndex >= 0 && this.selectedOptionIndex < visibleOptions.length) {
            const selectedOption = visibleOptions[this.selectedOptionIndex];
            selectedOption.classList.add("apollos-mode-dropdown-option-selected");
    ...
    async deleteMessage(messageEl: HTMLElement, skipPaired: boolean = false, skipBackend: boolean = false) {
        ...
        const messageContainer = messageEl.closest('.apollos-chat-message');
        if (!messageContainer) return;

        // Get paired message to delete if needed
        let pairedMessageContainer: Element | null = null;
        if (!skipPaired) {
            const messages = Array.from(document.getElementsByClassName('apollos-chat-message'));
            const currentIndex = messages.indexOf(messageContainer as HTMLElement);

            // If we're deleting a user message, also delete the subsequent apollos message (if any)
            if (messageContainer.classList.contains('you') && currentIndex < messages.length - 1) {
                pairedMessageContainer = messages[currentIndex + 1];
            }
            // If we're deleting a apollos message, also delete the preceding user message (if any)
            else if (messageContainer.classList.contains('apollos') && currentIndex > 0) {
                pairedMessageContainer = messages[currentIndex - 1];
            }
        }
        ...
        setTimeout(async () => {
            ...
            if (!skipBackend && turnId) {
                const chatBodyEl = this.contentEl.getElementsByClassName("apollos-chat-body")[0] as HTMLElement;
                ...
                try {
                    // Delete from backend
                    const response = await fetch(`${this.setting.apollosUrl}/api/chat/conversation/message`, {
                        method: 'DELETE',
                        headers: {
                            'Content-Type': 'application/json',
                            'Authorization': `Bearer ${this.setting.apollosApiKey}`
    ...
    async fetchAgents() {
        try {
            const response = await fetch(`${this.setting.apollosUrl}/api/agents`, {
                headers: {
                    "Authorization": `Bearer ${this.setting.apollosApiKey}`
                }
            });
    ...
    private async applyEditBlocks(editBlocks: EditBlock[]) {
        ...
        const chatBodyEl = this.contentEl.getElementsByClassName("apollos-chat-body")[0];
    ...
    // Add this new method to handle apollos-edit block transformation
    ...
    private async handleEditRetry(errorBlock: EditBlock) {
        ...
        const retryContainer = chatBodyEl.createDiv({ cls: "apollos-retry-container" });

        // Create the retry badge inside the container
        const retryBadge = retryContainer.createDiv({ cls: "apollos-retry-badge" });
        ...
        if (!this.modeDropdown) {
            ...
            this.chatModes.forEach((mode, index) => {
                const option = this.modeDropdown!.createDiv({
                    cls: "apollos-mode-dropdown-option",
                    attr: {
                ...
                const emojiSpan = option.createSpan({
                    cls: "apollos-mode-dropdown-emoji"
                });
                setIcon(emojiSpan, mode.iconName);

                option.createSpan({
                    cls: "apollos-mode-dropdown-label",
                    text: ` ${mode.label} `
                });

                option.createSpan({
                    cls: "apollos-mode-dropdown-command",
                    text: `(${mode.command})`
                });
