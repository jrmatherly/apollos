...
window.needsSubscriptionAPI.onNeedsSubscription((event, needsSubscription) => {
  ...
  if (needsSubscription) {
    window.alert(
      "Looks like you're out of space to sync your files. Upgrade your plan to unlock more space here: https://app.apollosai.dev/settings#subscription",
    );
    needsSubscriptionElement.style.display = "block";
  }
});

const urlInput = document.getElementById("apollos-host-url");
(async function () {
...
const apollosKeyInput = document.getElementById("apollos-access-key");
(async function () {
  const token = await window.tokenAPI.getToken();
  apollosKeyInput.value = token;
})();

apollosKeyInput.addEventListener("blur", async () => {
  const token = await window.tokenAPI.setToken(apollosKeyInput.value.trim());
  apollosKeyInput.value = token;
});
