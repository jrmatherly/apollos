...
function generateReference(referenceJson, index) {
  let reference = referenceJson.hasOwnProperty("compiled")
  ...
  short_ref =
    short_ref.length < escaped_ref.length ? short_ref + "..." : short_ref;
...
function renderMessage(
  ...
  let by_name = by == "apollos" ? "🏮 Apollos" : "🤔 You";
...
function renderMessageWithReference(
  message,
  by,
  context = null,
  dt = null,
  onlineContext = null,
  intentType = null,
  inferredQueries = null,
  conversationId = null,
  hostURL = null,
) {
  let chatEl;
  if (intentType?.includes("text-to-image")) {
    let imageMarkdown = generateImageMarkdown(
      message,
      intentType,
      inferredQueries,
    );
    ...
    let domain = hostURL ?? "https://app.apollosai.dev/";
...
function createReferenceSection(references, createLinkerSection = false) {
  ...
  if (references.hasOwnProperty("online")) {
    numReferences += processOnlineReferences(
      referenceSection,
      references["online"],
    );
  }
...
function handleStreamResponse(
  ...
  if (
    newResponseElement.getElementsByClassName("lds-ellipsis").length > 0 &&
    loadingEllipsis
  )
  ...
  newResponseElement.appendChild(
    formatHTMLMessage(rawResponse, false, replace, rawQuery),
  );
...
function convertMessageChunkToJson(rawChunk) {
  ...
  if (rawChunk?.startsWith("{") && rawChunk?.endsWith("}")) {
    try {
      ...
      if (!jsonChunk.type) jsonChunk = { type: "message", data: jsonChunk };
      return jsonChunk;
    } catch (e) {
...
function processMessageChunk(rawChunk) {
  ...
  if (chunk.type === "status") {
    ...
    handleStreamResponse(
      chatMessageState.newResponseTextEl,
      statusMessage,
      chatMessageState.rawQuery,
      chatMessageState.loadingEllipsis,
      false,
    );
...
function handleJsonResponse(jsonData) {
  if (jsonData.image || jsonData.detail) {
    chatMessageState.rawResponse = handleImageResponse(
      jsonData,
      chatMessageState.rawResponse,
    );
  ...
  if (chatMessageState.newResponseTextEl) {
    ...
    chatMessageState.newResponseTextEl.appendChild(
