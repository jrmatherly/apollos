// NOTE: Domain references below are hardcoded. If forking this project, update apollosai.dev to your domain.
const {
  app,
  BrowserWindow,
  ipcMain,
...
const apollosPackage = require("./package.json");
...
const APOLLOS_URL = "https://app.apollosai.dev";

const Store = require("electron-store");

const textFileTypes = [
  // Default valid file extensions supported by Apollos
  "org",
  "md",
  "markdown",
  "txt",
...
function filenameToMimeType(filename) {
  ...
  switch (extension) {
    ...
    default:
      console.warn(
        `Unknown file type: ${extension}. Defaulting to text/plain.`,
      );
      return "text/plain";
  }
}
...
function processDirectory(filesToPush, folder) {
  try {
    ...
    if (err.code === "EACCES") {
      ...
      console.error(
        `An error occurred while reading directory: ${error.message}`,
      );
    }
    return;
  }
...
function pushDataToApollos(regenerate = false) {
  // Don't sync if token or hostURL is not set or if already syncing
  if (
    store.get("apollosToken") === "" ||
    store.get("hostURL") === "" ||
    syncing === true
  ) {
  ...
  for (const file of filesToPush) {
    ...
    // Collect all updated or newly created files since last sync to index on Apollos server
    try {
      let encoding = binaryFileTypes.includes(file.split(".").pop())
        ? "binary"
        : "utf8";
      let mimeType =
        filenameToMimeType(file) +
        (encoding === "utf8" ? "; charset=UTF-8" : "");
      let fileContent = Buffer.from(
        fs.readFileSync(file, { encoding: encoding }),
        encoding,
      );
  ...
  // Mark deleted files for removal from index on Apollos server
  ...
  // Send collected files to Apollos server for indexing
  const hostURL = store.get("hostURL") || APOLLOS_URL;
  const headers = { Authorization: `Bearer ${store.get("apollosToken")}` };
  let requests = [];
  ...
  for (let i = 0; i < filesDataToPush.length; i += 1000) {
    ...
    filesDataGroup.forEach((fileData) => {
  ...
  Promise.all(requests)
    .then((responses) => {
      const lastSync = filesToPush
        .filter((file) =>
          responses.find((response) => response.data.includes(file)),
        )
    ...
    .catch((error) => {
      ...
      if (
        ...
        state["error"] =
          `Looks like you're out of space to sync your files. <a href="https://app.apollosai.dev/settings#subscription">Upgrade your plan</a> to unlock more space.`;
        ...
        state["error"] =
          `Could not connect to Apollos server. Ensure you can connect to it at ${error.address}:${error.port}.`;
      } else {
        currentTime = new Date();
        state["error"] =
          `Sync was unsuccessful at ${currentTime.toLocaleTimeString()}. Contact placeholder@apollosai.dev to report this issue.`;
      }
    })
    .finally(() => {
      // Syncing complete
...
async function handleFileOpen(type) {
  ...
  if (type === "file") {
    ({ canceled, filePaths } = await dialog.showOpenDialog({
      properties: ["openFile"],
      filters: [{ name: "Valid Apollos Files", extensions: validFileTypes }],
    }));
  } else if (type === "folder") {
    ({ canceled, filePaths } = await dialog.showOpenDialog({
...
async function getToken() {
  return store.get("apollosToken");
}

async function setToken(event, token) {
  store.set("apollosToken", token);
  return store.get("apollosToken");
}
...
async function syncData(regenerate = false) {
  try {
    pushDataToApollos(regenerate);
    const date = new Date();
    console.log("Pushing data to Apollos at: ", date);
  } catch (err) {
    console.error(err);
  }
}
...
// Fetch user info from Apollos server
async function getUserInfo() {
  const getUserInfoURL = `${store.get("hostURL") || APOLLOS_URL}/api/v1/user?client=desktop`;
  const headers = { Authorization: `Bearer ${store.get("apollosToken")}` };
  try {
...
function addCSPHeaderToSession() {
  ...
  const defaultDomains = `'self' ${hostURL} https://app.apollosai.dev https://assets.apollosai.dev`;
  ...
  const img_src = `img-src ${defaultDomains} data: https://*.apollosai.dev https://*.googleusercontent.com;`;
...
app.whenReady().then(() => {
  ...
  app.setAboutPanelOptions({
    applicationName: "Apollos",
    applicationVersion: apollosPackage.version,
    version: apollosPackage.version,
    authors: "Apollos AI",
    website: "https://apollosai.dev",
    copyright: "GPL v3",
...
function openAboutWindow() {
  ...
  // Pass OS, Apollos version to About page
  aboutWindow.webContents.on("did-finish-load", () => {
    aboutWindow.webContents.send("appInfo", {
      version: apollosPackage.version,
