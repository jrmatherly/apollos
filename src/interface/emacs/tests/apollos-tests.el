;;; apollos-tests.el --- Test suite for apollos.el  -*- lexical-binding: t -*-
...
;; This file contains the test suite for apollos.el.
...
(ert-deftest apollos-tests--extract-entries-as-markdown ()
  "Test `json-response', `query' from API formatted as markdown."
  (let ((user-query "Become God")
        (json-response-from-apollos-backend
...
")))
    (should
     (equal
      (apollos--extract-entries-as-markdown json-response-from-apollos-backend user-query nil)
      "\
# Become God\n\
## Upgrade\n\
\n\
...
(ert-deftest apollos-tests--extract-entries-as-org ()
...
")))
    (should
     (equal
      (apollos--extract-entries-as-org json-response-from-apollos-backend user-query nil)
      "\
* Become God\n\
** Upgrade\n\
\n\
...
(ert-deftest apollos-tests--get-current-outline-entry-text ()
...
(ert-deftest apollos-tests--get-current-paragraph-text ()
...
(ert-deftest apollos-tests--render-files-to-add-request-body ()
  ...
  (let ((upgrade-file (make-temp-file "upgrade" nil ".org" "# Become God\n## Upgrade\n\nPenance to Immortality\n\n"))
    ...
    (unwind-protect
        (progn
          (should
           (equal
            (apollos--render-update-files-as-request-body (list upgrade-file act-file) "apollos")
            (format
             "\n--apollos\r\n\
...
(ert-deftest apollos-tests--render-files-to-add-delete-in-request-body ()
  "Test files are formatted into a multi-part http request body"
  (let ((upgrade-file (make-temp-file "upgrade" nil ".org" "# Become God\n## Upgrade\n\nPenance to Immortality\n\n"))
        (act-file (make-temp-file "act" nil ".org" "## Act\n\nRule everything\n\n")))
    (unwind-protect
        (progn
          (should
           (equal
            (apollos--render-delete-files-as-request-body (list upgrade-file act-file "/tmp/deleted-file.org") "apollos")
            (format
             "\n--apollos\r\n\
Content-Disposition: form-data; name=\"files\"; filename=\"%s\"\r\n\
Content-Type: text/org\r\n\r\n\
...
(ert-deftest apollos-tests--extract-online-references ()
  (let* (;; Arrange
         ...
         ;; Act
         (result (apollos--extract-online-references
...
(provide 'apollos-tests)

;;; apollos-tests.el ends here
