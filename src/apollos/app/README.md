...
Using the `docker-compose.yml` file in the root directory, you can run the Apollos app using the following command:
...
```bash
createdb apollos -U postgres
```

#### Debian, Ubuntu

```bash
sudo -u postgres createdb apollos
```

- [Optional] To set default postgres user's password
  - Execute `ALTER USER postgres PASSWORD 'my_secure_password';` using `psql`
  - Run `export $POSTGRES_PASSWORD=my_secure_password` in your terminal for Apollos to use it later
...
```bash
python3 src/apollos/main.py --anonymous-mode
```
