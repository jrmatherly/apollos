...
from apollos.configure import initialize_server
from apollos.database.adapters import UserMemoryAdapters
from apollos.database.models import (
    ApollosUser,
    Conversation,
    DataStore,
)
from apollos.routers.helpers import extract_facts_from_query
...
class Command(BaseCommand):
    ...
    async def get_users_to_process(self, users_str: Optional[str]) -> List[ApollosUser]:
        ...
        if users_str:
            ...
            users = [
                user async for user in ApollosUser.objects.filter(Q(username__in=usernames) | Q(email__in=usernames))
            ]
            return users
        else:
            # Process all users with conversations
            return [user async for user in ApollosUser.objects.filter(conversation__isnull=False).distinct()]

    async def get_user_conversations(
        self, user: ApollosUser, cutoff_date: Optional[datetime], checkpoint: dict, force: bool
    ) -> List[Conversation]:
    ...
    async def process_conversation_batch(
        self, user: ApollosUser, conversations: List[Conversation], apply: bool, checkpoint: dict
    ) -> int:
        ...
        for conversation in conversations:
            try:
                ...
                while i + 1 < len(messages):
                    ...
                    if messages[i].by != "you" or messages[i + 1].by != "apollos":
                        i += 1
                        continue
    ...
    async def handle_delete_memories(self, usernames: Optional[str], cutoff_date: Optional[datetime], apply: bool):
        ...
        from apollos.database.models import UserMemory
