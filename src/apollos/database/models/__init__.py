...
class ApollosUser(AbstractUser):
...
class GoogleUser(models.Model):
    user = models.OneToOneField(ApollosUser, on_delete=models.CASCADE)
...
class ApollosApiUser(models.Model):
    """User issued API tokens to authenticate Apollos clients"""

    user = models.ForeignKey(ApollosUser, on_delete=models.CASCADE)
...
class Subscription(DbBaseModel):
    ...
    user = models.OneToOneField(ApollosUser, on_delete=models.CASCADE, related_name="subscription")
...
class Agent(DbBaseModel):
    ...
    creator = models.ForeignKey(
        ApollosUser, on_delete=models.CASCADE, default=None, null=True, blank=True
...
class Conversation(DbBaseModel):
    ...
    async def pop_message(self, interrupted: bool = False) -> Optional[ChatMessageModel]:
        ...
        When interrupted is True, we only drop the last message if it was an interrupted message by apollos.
        """
        ...
        is_interrupted_msg = last_message.get("by") == "apollos" and not last_message.get("message")
        # When handling an interruption, only pop if the last message is an empty one by apollos.
...
class PublicConversation(DbBaseModel):
    source_owner = models.ForeignKey(ApollosUser, on_delete=models.CASCADE)
    conversation_log = models.JSONField(default=dict)
    slug = models.CharField(max_length=200, default=None, null=True, blank=True)
...
class ReflectiveQuestion(DbBaseModel):
    ...
    user = models.ForeignKey(ApollosUser, on_delete=models.CASCADE, default=None, null=True, blank=True)
...
class DataStore(DbBaseModel):
    ...
    owner = models.ForeignKey(ApollosUser, on_delete=models.CASCADE, default=None, null=True, blank=True)
