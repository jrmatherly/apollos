...
from apollos.processor.conversation import prompts
...
from apollos.utils.helpers import (
    clean_object_for_db,
    clean_text_for_db,
...
def require_valid_user(func: Callable[P, T]) -> Callable[P, T]:
    ...
    def sync_wrapper(*args: P.args, **kwargs: P.kwargs) -> T:
        ...
        user = next((arg for arg in args if isinstance(arg, ApollosUser)), None)
        if not user:
            user = next((val for val in kwargs.values() if isinstance(val, ApollosUser)), None)

        # Throw error if user is not found
        if not user:
            raise ValueError("Apollos user argument required but not provided.")
...
def arequire_valid_user(func: Callable[P, Coroutine[Any, Any, T]]) -> Callable[P, Coroutine[Any, Any, T]]:
    ...
    async def async_wrapper(*args: P.args, **kwargs: P.kwargs) -> T:
        ...
        user = next((arg for arg in args if isinstance(arg, ApollosUser)), None)
        if not user:
            user = next((v for v in kwargs.values() if isinstance(v, ApollosUser)), None)
...
@arequire_valid_user
async def set_notion_config(token: str, user: ApollosUser):
...
@require_valid_user
def create_apollos_token(user: ApollosUser, name=None):
    "Create Apollos API key for user"
    token = f"kk-{secrets.token_urlsafe(32)}"
    name = name or f"{generate_random_name().title()}"
    return ApollosApiUser.objects.create(token=token, user=user, name=name)


@arequire_valid_user
async def acreate_apollos_token(user: ApollosUser, name=None):
    "Create Apollos API key for user"
    token = f"kk-{secrets.token_urlsafe(32)}"
    name = name or f"{generate_random_name().title()}"
    return await ApollosApiUser.objects.acreate(token=token, user=user, name=name)


@require_valid_user
def get_apollos_tokens(user: ApollosUser):
    "Get all Apollos API keys for user"
    return list(ApollosApiUser.objects.filter(user=user))


@arequire_valid_user
async def delete_apollos_token(user: ApollosUser, token: str):
    "Delete Apollos API Key for user"
    await ApollosApiUser.objects.filter(token=token, user=user).adelete()


async def get_or_create_user(token: dict) -> ApollosUser:
...
async def aget_or_create_user_by_phone_number(phone_number: str) -> tuple[ApollosUser, bool]:
    is_new = False
...
@arequire_valid_user
async def aset_user_phone_number(user: ApollosUser, phone_number: str) -> ApollosUser:
    if is_none_or_empty(phone_number):
        return None
...
@arequire_valid_user
async def aremove_phone_number(user: ApollosUser) -> ApollosUser:
...
async def acreate_user_by_phone_number(phone_number: str) -> ApollosUser:
    if is_none_or_empty(phone_number):
        return None
    user, _ = await ApollosUser.objects.filter(phone_number=phone_number).aupdate_or_create(
...
async def aget_or_create_user_by_email(input_email: str, check_deliverability=False) -> tuple[ApollosUser, bool]:
    ...
    is_existing_user = await ApollosUser.objects.filter(email=email).aexists()
    ...
    user, is_new = await ApollosUser.objects.filter(email=email).aupdate_or_create(
...
async def aget_user_validated_by_email_verification_code(code: str, email: str) -> tuple[Optional[ApollosUser], bool]:
    # Normalize the email address
    normalized_email, _ = normalize_email(email)

    # Check if verification code exists for the user
    user = await ApollosUser.objects.filter(email_verification_code=code, email=normalized_email).afirst()
    if not user:
        return None, False
...
async def create_user_by_google_token(token: dict) -> ApollosUser:
    user, _ = await ApollosUser.objects.filter(email=token.get("email")).aupdate_or_create(
...
@require_valid_user
def set_user_name(user: ApollosUser, first_name: str, last_name: str) -> ApollosUser:
...
@require_valid_user
def get_user_name(user: ApollosUser):
...
@require_valid_user
def get_user_photo(user: ApollosUser):
    google_profile: GoogleUser = GoogleUser.objects.filter(user=user).first()
    if google_profile:
...
@arequire_valid_user
async def aget_user_subscription_state(user: ApollosUser) -> str:
    """Get subscription state of user
    Valid state transitions: trial -> subscribed <-> unsubscribed OR expired
    """
    user_subscription = await Subscription.objects.filter(user=user).afirst()
...
@arequire_valid_user
async def ais_user_subscribed(user: ApollosUser) -> bool:
    """
...
@require_valid_user
def is_user_subscribed(user: ApollosUser) -> bool:
    """
    Get whether the user is subscribed
    """
    if not state.billing_enabled or state.anonymous_mode:
...
async def aget_user_by_email(email: str) -> ApollosUser:
    return await ApollosUser.objects.filter(email=email).afirst()


def get_user_by_email(email: str) -> ApollosUser:
    return ApollosUser.objects.filter(email=email).first()


async def aget_user_by_uuid(uuid: str) -> ApollosUser:
    return await ApollosUser.objects.filter(uuid=uuid).afirst()


async def get_user_by_token(token: dict) -> ApollosUser:
...
async def aget_user_by_phone_number(phone_number: str) -> ApollosUser:
    if is_none_or_empty(phone_number):
        return None
    matched_user = await ApollosUser.objects.filter(phone_number=phone_number).prefetch_related("subscription").afirst()

    if not matched_user:
        return None

    # If the user with this phone number does not have an email account with Apollos, return the user
    if is_none_or_empty(matched_user.email):
        return matched_user

    # If the user has an email account with Apollos and a verified number, return the user
...
async def retrieve_user(session_id: str) -> ApollosUser:
    ...
    user = await ApollosUser.objects.filter(id=session_data.get("_auth_user_id")).afirst()
    if not user:
        raise HTTPException(status_code=401, detail="Invalid user")
    return user


def get_all_users() -> BaseManager[ApollosUser]:
    return ApollosUser.objects.all()


@require_valid_user
def get_user_github_config(user: ApollosUser):
    config = GithubConfig.objects.filter(user=user).prefetch_related("githubrepoconfig").first()
    return config


@require_valid_user
def get_user_notion_config(user: ApollosUser):
...
@arequire_valid_user
async def aget_user_name(user: ApollosUser):
    full_name = user.get_full_name()
    if not is_none_or_empty(full_name):
        return full_name
...
@arequire_valid_user
async def set_user_github_config(user: ApollosUser, pat_token: str, repos: list):
...
class AgentAdapters:
    DEFAULT_AGENT_NAME = "Apollos"
    DEFAULT_AGENT_SLUG = "apollos"
    ...
    @staticmethod
    @arequire_valid_user
    async def adelete_agent_by_slug(agent_slug: str, user: ApollosUser):
    ...
    @staticmethod
    async def aget_all_accessible_agents(user: ApollosUser = None) -> List[Agent]:
    ...
    @staticmethod
    def get_agent_chat_model(agent: Agent, user: Optional[ApollosUser]) -> Optional[ChatModel]:
        """
    ...
    @staticmethod
    async def aget_agent_chat_model(agent: Agent, user: Optional[ApollosUser]) -> Optional[ChatModel]:
...
class PublicConversationAdapters:
    ...
    @staticmethod
    def delete_public_conversation_by_slug(user: ApollosUser, slug: str):
...
class ConversationAdapters:
    ...
    def get_conversation_by_user(
        user: ApollosUser, client_application: ClientApplication = None, conversation_id: str = None
    ...
    @staticmethod
    @require_valid_user
    def get_all_conversations_for_export(user: ApollosUser, page: Optional[int] = 0):
        all_conversations = Conversation.objects.filter(user=user).prefetch_related("agent")[page : page + 10]
        histories = []
        for conversation in all_conversations:
            history = {
                "title": conversation.title,
                "agent": conversation.agent.name if conversation.agent else "Apollos",
    ...
    @staticmethod
    @require_valid_user
    def get_num_conversations(user: ApollosUser):
        return Conversation.objects.filter(user=user).count()

    @staticmethod
    @require_valid_user
    def get_conversation_sessions(user: ApollosUser, client_application: ClientApplication = None):
        return (
    ...
    async def aset_conversation_title(
        user: ApollosUser, client_application: ClientApplication, conversation_id: str, title: str
    ):
    ...
    async def acreate_conversation_session(
        user: ApollosUser, client_application: ClientApplication = None, agent_slug: str = None, title: str = None
    ):
        if agent_slug:
    ...
    @staticmethod
    @require_valid_user
    def has_any_chat_model(user: ApollosUser):
    ...
    @staticmethod
    @arequire_valid_user
    async def aset_user_conversation_processor(user: ApollosUser, conversation_processor_config_id: int):
    ...
    @staticmethod
    @arequire_valid_user
    async def aset_user_voice_model(user: ApollosUser, model_id: str):
    ...
    @staticmethod
    def get_chat_model(user: ApollosUser):
    ...
    @staticmethod
    async def aget_chat_model(user: ApollosUser):
    ...
    @staticmethod
    async def aget_voice_model_config(user: ApollosUser) -> Optional[VoiceModelOption]:
    ...
    @staticmethod
    def get_voice_model_config(user: ApollosUser) -> Optional[VoiceModelOption]:
    ...
    @staticmethod
    def get_default_chat_model(user: ApollosUser = None):
    ...
    async def aget_default_chat_model(
        user: ApollosUser = None, fallback_chat_model: Optional[ChatModel] = None, fast: Optional[bool] = None
    ):
        """
    ...
    @staticmethod
    def get_advanced_chat_model(user: ApollosUser):
        server_chat_settings = ServerChatSettings.objects.first()
    ...
    @staticmethod
    async def aget_advanced_chat_model(user: ApollosUser = None):
        server_chat_settings: ServerChatSettings = (
            await ServerChatSettings.objects.filter()
    ...
    @staticmethod
    def get_max_context_size(chat_model: ChatModel, user: ApollosUser) -> int | None:
    ...
    @staticmethod
    async def aget_max_context_size(chat_model: ChatModel, user: ApollosUser) -> int | None:
        """Get the max context size for the user based on the chat model."""
    ...
    @staticmethod
    async def aget_chat_model_slot(user: ApollosUser = None, fast: Optional[bool] = None):
        """
    ...
    def create_conversation_from_public_conversation(
        user: ApollosUser, public_conversation: PublicConversation, client_app: ClientApplication
    ):
    ...
    @staticmethod
    def set_user_chat_model(user: ApollosUser, chat_model: ChatModel):
    ...
    @staticmethod
    async def aget_user_chat_model(user: ApollosUser):
        config = (
            await UserConversationConfig.objects.filter(user=user).prefetch_related("setting__ai_model_api").afirst()
        )
        if not config:
    ...
    @staticmethod
    async def ais_memory_enabled(user: ApollosUser) -> bool:
        """
    ...
    @staticmethod
    def is_memory_enabled(user: ApollosUser) -> bool:
        """
    ...
    @staticmethod
    @arequire_valid_user
    async def aget_conversation_starters(user: ApollosUser, max_results=3):
        all_questions = []
    ...
    @staticmethod
    async def aget_valid_chat_model(user: ApollosUser, conversation: Conversation, is_subscribed: bool):
        """
    ...
    @staticmethod
    def get_user_text_to_image_model_config(user: ApollosUser):
    ...
    @staticmethod
    async def aget_user_text_to_image_model(user: ApollosUser) -> Optional[TextToImageModelConfig]:
    ...
    @staticmethod
    async def aset_user_text_to_image_model(user: ApollosUser, text_to_image_model_config_id: int):
    ...
    @staticmethod
    def add_files_to_filter(user: ApollosUser, conversation_id: str, files: List[str]):
    ...
    @staticmethod
    def remove_files_from_filter(user: ApollosUser, conversation_id: str, files: List[str]):
        conversation = ConversationAdapters.get_conversation_by_user(user, conversation_id=conversation_id)
        if not conversation:
            return []
        for filename in files:
    ...
    @staticmethod
    @require_valid_user
    def delete_message_by_turn_id(user: ApollosUser, conversation_id: str, turn_id: str):
        conversation = ConversationAdapters.get_conversation_by_user(user, conversation_id=conversation_id)
...
class FileObjectAdapters:
    ...
    @staticmethod
    @require_valid_user
    def create_file_object(user: ApollosUser, file_name: str, raw_text: str):
    ...
    @staticmethod
    @require_valid_user
    def get_file_object_by_name(user: ApollosUser, file_name: str):
        return FileObject.objects.filter(user=user, file_name=file_name).first()

    @staticmethod
    @require_valid_user
    def get_all_file_objects(user: ApollosUser):
        return FileObject.objects.filter(user=user).all()

    @staticmethod
    @require_valid_user
    def delete_file_object_by_name(user: ApollosUser, file_name: str):
        return FileObject.objects.filter(user=user, file_name=file_name).delete()

    @staticmethod
    @require_valid_user
    def delete_all_file_objects(user: ApollosUser):
    ...
    @staticmethod
    @arequire_valid_user
    async def acreate_file_object(user: ApollosUser, file_name: str, raw_text: str):
        cleaned_raw_text = clean_text_for_db(raw_text)
        return await FileObject.objects.acreate(user=user, file_name=file_name, raw_text=cleaned_raw_text)

    @staticmethod
    @arequire_valid_user
    async def aget_file_objects_by_name(user: ApollosUser, file_name: str, agent: Agent = None):
        return await sync_to_async(list)(FileObject.objects.filter(user=user, file_name=file_name, agent=agent))

    @staticmethod
    @arequire_valid_user
    async def aget_file_objects_by_path_prefix(user: ApollosUser, path_prefix: str, agent: Agent = None):
    ...
    @staticmethod
    @arequire_valid_user
    async def aget_file_objects_by_names(user: ApollosUser, file_names: List[str]):
        return await sync_to_async(list)(FileObject.objects.filter(user=user, file_name__in=file_names))

    @staticmethod
    @require_valid_user
    async def aget_all_file_objects(user: ApollosUser, start: int = 0, limit: int = 10):
        query = FileObject.objects.filter(user=user).order_by("-updated_at")[start : start + limit]
        return await sync_to_async(list)(query)

    @staticmethod
    @require_valid_user
    async def aget_number_of_pages(user: ApollosUser, limit: int = 10):
        count = await FileObject.objects.filter(user=user).acount()
        return math.ceil(count / limit)

    @staticmethod
    @arequire_valid_user
    async def adelete_file_object_by_name(user: ApollosUser, file_name: str):
        return await FileObject.objects.filter(user=user, file_name=file_name).adelete()

    @staticmethod
    @arequire_valid_user
    async def adelete_file_objects_by_names(user: ApollosUser, file_names: List[str]):
        return await FileObject.objects.filter(user=user, file_name__in=file_names).adelete()

    @staticmethod
    @arequire_valid_user
    async def adelete_all_file_objects(user: ApollosUser):
        return await FileObject.objects.filter(user=user).adelete()

    @staticmethod
    @arequire_valid_user
    async def aget_file_objects_by_regex(user: ApollosUser, regex_pattern: str, path_prefix: Optional[str] = None):
        """
...
class EntryAdapters:
    ...
    @staticmethod
    @require_valid_user
    def does_entry_exist(user: ApollosUser, hashed_value: str) -> bool:
        return Entry.objects.filter(user=user, hashed_value=hashed_value).exists()

    @staticmethod
    @require_valid_user
    def delete_entry_by_file(user: ApollosUser, file_path: str):
        deleted_count, _ = Entry.objects.filter(user=user, file_path=file_path).delete()
        return deleted_count

    @staticmethod
    @require_valid_user
    def get_filtered_entries(user: ApollosUser, file_type: str = None, file_source: str = None):
    ...
    @staticmethod
    @require_valid_user
    def delete_all_entries(user: ApollosUser, file_type: str = None, file_source: str = None, batch_size=1000):
        deleted_count = 0
    ...
    @staticmethod
    @arequire_valid_user
    async def adelete_all_entries(user: ApollosUser, file_type: str = None, file_source: str = None, batch_size=1000):
        deleted_count = 0
        queryset = EntryAdapters.get_filtered_entries(user, file_type, file_source)
    ...
    @staticmethod
    @require_valid_user
    def get_existing_entry_hashes_by_file(user: ApollosUser, file_path: str):
        return Entry.objects.filter(user=user, file_path=file_path).values_list("hashed_value", flat=True)

    @staticmethod
    @require_valid_user
    def delete_entry_by_hash(user: ApollosUser, hashed_values: List[str]):
    ...
    @staticmethod
    @require_valid_user
    def user_has_entries(user: ApollosUser):
    ...
    @staticmethod
    @arequire_valid_user
    async def auser_has_entries(user: ApollosUser):
    ...
    @staticmethod
    @arequire_valid_user
    async def adelete_entry_by_file(user: ApollosUser, file_path: str):
        return await Entry.objects.filter(user=user, file_path=file_path).adelete()

    @staticmethod
    @arequire_valid_user
    async def adelete_entries_by_filenames(user: ApollosUser, filenames: List[str], batch_size=1000):
        deleted_count = 0
    ...
    @staticmethod
    @require_valid_user
    def get_all_filenames_by_source(user: ApollosUser, file_source: str):
        return (
    ...
    @staticmethod
    @require_valid_user
    def get_all_filenames_by_type(user: ApollosUser, file_type: str):
        return (
    ...
    @staticmethod
    @require_valid_user
    def get_size_of_indexed_data_in_mb(user: ApollosUser):
        entries = Entry.objects.filter(user=user).iterator()
        total_size = sum(sys.getsizeof(entry.compiled) for entry in entries)
        return total_size / 1024 / 1024

    @staticmethod
    def apply_filters(user: ApollosUser, query: str, file_type_filter: str = None, agent: Agent = None):
        q_filter_terms = Q()
...
class AutomationAdapters:
    ...
    @staticmethod
    def get_automation_metadata(user: ApollosUser, automation: Job):
    ...
    @staticmethod
    def get_job_last_run(user: ApollosUser, automation: Job):
        # Perform validation checks
        # Check if user is allowed to delete this automation id
        if not automation.id.startswith(f"automation_{user.uuid}_"):
            raise ValueError(f"Invalid automation id: {automation.id}")
    ...
    @staticmethod
    def get_automations_metadata(user: ApollosUser):
    ...
    @staticmethod
    def get_automation(user: ApollosUser, automation_id: str) -> Job:
        # Perform validation checks
    ...
    @staticmethod
    async def aget_automation(user: ApollosUser, automation_id: str) -> Job:
        # Perform validation checks
        # Check if user is allowed to retrieve this automation id
        if is_none_or_empty(automation_id) or not automation_id.startswith(f"automation_{user.uuid}_"):
            raise ValueError(f"Invalid automation id: {automation_id}")
    ...
    @staticmethod
    def delete_automation(user: ApollosUser, automation_id: str):
...
class UserMemoryAdapters:
    @staticmethod
    @require_valid_user
    async def pull_memories(user: ApollosUser, agent: Agent = None, limit=10, window=7) -> list[UserMemory]:
        """
    ...
    @staticmethod
    @require_valid_user
    async def save_memory(user: ApollosUser, memory: str, agent: Agent = None) -> UserMemory:
        """
    ...
    @staticmethod
    @require_valid_user
    async def search_memories(query: str, user: ApollosUser, agent: Agent = None, limit: int = 10) -> list[UserMemory]:
        """
    ...
    @staticmethod
    @require_valid_user
    async def delete_memory(user: ApollosUser, memory_id: str) -> bool:
        """
