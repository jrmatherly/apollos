...
from apollos.database.models import Subscription
from apollos.routers.helpers import update_telemetry_state
from apollos.utils import state

# Stripe integration for Apollos Cloud Subscription
if state.billing_enabled:
    import stripe

    stripe.api_key = os.getenv("STRIPE_API_KEY")
endpoint_secret = os.getenv("STRIPE_SIGNING_SECRET")
official_product_id = os.getenv("STRIPE_APOLLOS_PRODUCT_ID", "")
...
async def subscribe(request: Request):
    """Webhook for Stripe to send subscription events to Apollos Cloud"""
    event = None
    try:
