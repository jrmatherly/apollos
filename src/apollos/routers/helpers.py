...
from apollos.processor.content.github.github_to_entries import GithubToEntries
from apollos.processor.content.images.image_to_entries import ImageToEntries
from apollos.processor.content.markdown.markdown_to_entries import MarkdownToEntries
from apollos.processor.content.notion.notion_to_entries import NotionToEntries
from apollos.processor.content.org_mode.org_to_entries import OrgToEntries
from apollos.processor.content.pdf.pdf_to_entries import PdfToEntries
from apollos.processor.content.plaintext.plaintext_to_entries import PlaintextToEntries
from apollos.processor.conversation import prompts
from apollos.processor.conversation.anthropic.anthropic_chat import (
    anthropic_send_message_to_model,
    converse_anthropic,
)
from apollos.processor.conversation.google.gemini_chat import (
    converse_gemini,
    gemini_send_message_to_model,
)
from apollos.processor.conversation.openai.gpt import (
    converse_openai,
    openai_send_message_to_model,
)
from apollos.processor.conversation.utils import (
    ChatEvent,
    OperatorRun,
    ResearchIteration,
    ResponseWithThought,
...
from apollos.processor.speech.text_to_speech import is_eleven_labs_enabled
from apollos.routers.email import is_resend_enabled, send_task_email
from apollos.routers.twilio import is_twilio_enabled
...
from apollos.search_type import text_search
...
from apollos.utils.rawconfig import (
    ChatRequestBody,
    FileData,
    LocationData,
    SearchResponse,
...
from apollos.utils.yaml import yaml_dump
...
def validate_chat_model(user: ApollosUser):
...
async def is_ready_to_chat(user: ApollosUser):
    ...
    raise HTTPException(status_code=500, detail="Set your OpenAI API key or enable Local LLM via Apollos settings.")
...
def update_telemetry_state(
    ...
    user: ApollosUser = request.user.object if request.user.is_authenticated else None
...
async def acreate_title_from_query(query: str, user: ApollosUser = None) -> str:
    """
...
async def acheck_if_safe_prompt(system_prompt: str, user: ApollosUser = None, lax: bool = False) -> Tuple[bool, str]:
    """
...
def schedule_query(
    q: str, chat_history: List[ChatMessageModel], user: ApollosUser, query_images: List[str] = None, tracer: dict = {}
) -> Tuple[str, str, str]:
    """
...
async def search_documents(
    ...
    if not await sync_to_async(EntryAdapters.user_has_entries)(user=user):
        if not agent_has_entries:
            logger.debug("No documents in knowledge base. Use a Apollos client to sync and chat with your docs.")
            yield compiled_references, inferred_queries, q
            return
...
class EmailVerificationApiRateLimiter:
    ...
    async def __call__(self, email: str = None):
        ...
        user: ApollosUser = await aget_user_by_email(email)
        if not user:
...
class ApiUserRateLimiter:
    ...
    async def check_websocket(self, websocket: WebSocket):
        ...
        user: ApollosUser = websocket.scope["user"].object
...
class WebSocketConnectionManager:
    ...
    async def register_connection(self, user: ApollosUser, connection_id: str) -> None:
        """Register a new WebSocket connection."""
        await UserRequests.objects.acreate(user=user, slug=f"{self.connection_slug_prefix}{connection_id}")

    async def unregister_connection(self, user: ApollosUser, connection_id: str) -> None:
    ...
    async def _cleanup_stale_connections(self, user: ApollosUser) -> None:
...
def format_automation_response(
    scheduling_request: str, executed_query: str, ai_response: str, user: ApollosUser
) -> bool:
    """
...
def should_notify(original_query: str, executed_query: str, ai_response: str, user: ApollosUser) -> bool:
    """
...
def scheduled_chat(
    ...
    url = f"{scheme}://{parsed_url.netloc}/api/chat?client=apollos"

    # Construct the Headers for the chat API
    headers = {"User-Agent": "Apollos", "Content-Type": "application/json"}
    if not state.anonymous_mode:
        # Add authorization request header in non-anonymous mode
        token = get_apollos_tokens(user)
        if is_none_or_empty(token):
            token = create_apollos_token(user).token
        else:
            token = token[0].token
...
def get_user_config(user: ApollosUser, request: Request, is_detailed: bool = False):
    ...
    return {
        ...
        # server settings
        "apollos_cloud_subscription_url": os.getenv("APOLLOS_CLOUD_SUBSCRIPTION_URL"),
...
def get_notion_auth_url(user: ApollosUser):
