...
from apollos.app.settings import DISABLE_HTTPS
from apollos.database.adapters import (
    acreate_apollos_token,
    aget_or_create_user_by_email,
    aget_user_validated_by_email_verification_code,
    delete_apollos_token,
    get_apollos_tokens,
    get_or_create_user,
)
from apollos.routers.email import send_magic_link_email, send_welcome_email
...
from apollos.utils.helpers import in_debug_mode
...
if not state.anonymous_mode:
    ...
    if missing_requirements:
        ...
        error_msg = f"🚨 Start Apollos with --anonymous-mode flag or to enable authentication:{requirements_string}"
...
async def generate_token(request: Request, token_name: Optional[str] = None):
    ...
    if token_name:
        token = await acreate_apollos_token(user=request.user.object, name=token_name)
    else:
        token = await acreate_apollos_token(user=request.user.object)
    return {
        "token": token.token,
        "name": token.name,
    }
...
def get_tokens(request: Request):
    ...
    tokens = get_apollos_tokens(user=request.user.object)
    return tokens
...
async def delete_token(request: Request, token: str):
    ...
    return await delete_apollos_token(user=request.user.object, token=token)
...
async def auth_post(request: Request):
    ...
    apollos_user = await get_or_create_user(idinfo)

    if apollos_user:
        request.session["user"] = dict(idinfo)

        if datetime.timedelta(minutes=3) > (datetime.datetime.now(datetime.timezone.utc) - apollos_user.date_joined):
            asyncio.create_task(send_welcome_email(idinfo["name"], idinfo["email"]))
            update_telemetry_state(
                request=request,
                telemetry_type="api",
                api="create_user__google",
                metadata={"server_id": str(apollos_user.uuid)},
            )
            logger.log(logging.INFO, f"🥳 New User Created: {apollos_user.uuid}")
