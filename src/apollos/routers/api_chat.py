...
from apollos.app.settings import ALLOWED_HOSTS
...
from apollos.database.models import Agent, ApollosUser
from apollos.processor.conversation import prompts
from apollos.processor.conversation.openai.utils import is_local_api
from apollos.processor.conversation.prompts import no_entries_found
...
from apollos.processor.image.generate import text_to_image
from apollos.processor.operator import operate_environment
from apollos.processor.speech.text_to_speech import generate_text_to_speech
from apollos.processor.tools.online_search import (
    deduplicate_organic_results,
    read_webpages,
    search_online,
)
from apollos.processor.tools.run_code import run_code
from apollos.routers.email import send_query_feedback
...
from apollos.routers.research import ResearchIteration, research
from apollos.routers.storage import upload_user_image_to_bucket
...
def duplicate_chat_history_public_conversation(
    ...
    if scheme == "http" and not is_env_var_true("APOLLOS_NO_HTTPS") and not is_local_api(f"{request.base_url}"):
        scheme = "https"
...
async def event_generator(
    ...
    chat_metadata: dict = {}
    conversation = None
    user: ApollosUser = user_scope.object
