...
logger = logging.getLogger("apollos")
...
def shutdown_scheduler():
    logger.info("🌑 Shutting down Apollos")
...
def run(should_start_server=True):
    ...
    logger.info(f"🚒 Initializing Apollos v{state.apollos_version}")
...
def set_state(args):
    ...
    state.apollos_version = version("apollos")


def start_server(app, host=None, port=None, socket=None):
    logger.info("🌖 Apollos is ready to engage")
    if socket:
        uvicorn.run(
            app,
            proxy_headers=True,
