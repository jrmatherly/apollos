...
from apollos.utils import constants

if TYPE_CHECKING:
    from sentence_transformers import CrossEncoder, SentenceTransformer

    from apollos.utils.models import BaseEncoder
...
def get_server_id():
    ...
    Helps count distinct apollos servers deployed.
    Maintains anonymity by using non-PII random id."""
    # Initialize server_id to None
    server_id = None
    # Expand path to the apollos env file. It contains persistent internal app data
    ...
    if path.exists(app_env_filename):
        ...
        # Create apollos config directory if it doesn't exist
...
def log_telemetry(
    ...
    request_body = {
        ...
        "server_version": version("apollos"),
        "os": platform.system(),
...
command_descriptions = {
    ConversationCommand.General: "Only talk about information that relies on Apollos's general knowledge, not your personal knowledge base.",
...
tool_descriptions_for_llm = {
    ...
    ConversationCommand.Online: "To search for the latest, up-to-date information from the internet. Note: **Questions about Apollos should always use this data source**",
...
class ImageIntentType(Enum):
    """
    Chat message intent by Apollos for image responses.
...
def in_debug_mode():
    """Check if Apollos is running in debug mode.
    Set APOLLOS_DEBUG environment variable to true to enable debug mode."""
    return is_env_var_true("APOLLOS_DEBUG")


def is_promptrace_enabled():
    """Check if Apollos is running with prompt tracing enabled.
...
def is_operator_enabled():
    """Check if Apollos can operate GUI applications.
    Set APOLLOS_OPERATOR_ENABLED env var to true and install playwright to enable it."""
    return is_env_var_true("APOLLOS_OPERATOR_ENABLED")


def is_code_sandbox_enabled():
    """Check if Apollos can run code in sandbox.
    Set APOLLOS_TERRARIUM_URL or E2B api key via env var to enable it."""
    return not is_none_or_empty(os.getenv("APOLLOS_TERRARIUM_URL")) or is_e2b_code_sandbox_enabled()
