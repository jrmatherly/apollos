...
from apollos.utils.constants import (
...
def initialization(interactive: bool = True):
    def _create_admin_user():
        ...
        if not interactive and (not os.getenv("APOLLOS_ADMIN_EMAIL") or not os.getenv("APOLLOS_ADMIN_PASSWORD")):
            logger.error(
                "🚨 Admin user cannot be created. Please set the APOLLOS_ADMIN_EMAIL, APOLLOS_ADMIN_PASSWORD environment variables or start server in interactive mode."
            )
            exit(1)
        email_addr = os.getenv("APOLLOS_ADMIN_EMAIL") or input("Email: ")
        password = os.getenv("APOLLOS_ADMIN_PASSWORD") or input("Password: ")
        admin_user = ApollosUser.objects.create_superuser(email=email_addr, username=email_addr, password=password)
    ...
    admin_user = ApollosUser.objects.filter(is_staff=True).first()
