...
from apollos.database.models import ApollosUser, ClientApplication, ProcessLock, Subscription
...
from apollos.utils import constants, state
from apollos.utils.config import SearchType
from apollos.utils.helpers import is_none_or_empty
...
class UserAuthenticationBackend(AuthenticationBackend):
    def __init__(
        ...
        from apollos.database.models import ApollosApiUser, ApollosUser

        self.apollosuser_manager = ApollosUser.objects
        self.apollosapiuser_manager = ApollosApiUser.objects
        self._initialize_default_user()
        super().__init__()

    def _initialize_default_user(self):
        if not self.apollosuser_manager.filter(username="default").exists():
            default_user = self.apollosuser_manager.create_user(
                username="default",
    ...
    async def authenticate(self, request: HTTPConnection):
        ...
        if current_user and current_user.get("email"):
            try:
                user = (
                    await self.apollosuser_manager.filter(email=current_user.get("email"))
            ...
            if user:
                ...
                return AuthCredentials(["authenticated"]), AuthenticatedApollosUser(user)
        ...
        if len(request.headers.get("Authorization", "").split("Bearer ")) == 2:
            ...
            try:
                user_with_token = (
                    await self.apollosapiuser_manager.filter(token=bearer_token)
                    .select_related("user")
            ...
            if user_with_token:
                ...
                if subscribed:
                    return AuthCredentials(["authenticated", "premium"]), AuthenticatedApollosUser(user_with_token.user)
                return AuthCredentials(["authenticated"]), AuthenticatedApollosUser(user_with_token.user)
        ...
        if client_id:
            ...
            if subscribed:
                return AuthCredentials(["authenticated", "premium"]), AuthenticatedApollosUser(user, client_application)
            return AuthCredentials(["authenticated"]), AuthenticatedApollosUser(user, client_application)

        # No auth required if server in anonymous mode
        if state.anonymous_mode:
            try:
                user = (
                    await self.apollosuser_manager.filter(username="default").prefetch_related("subscription").afirst()
                )
            except (DatabaseError, OperationalError):
...
def initialize_content(user: ApollosUser, regenerate: bool, search_type: Optional[SearchType] = None):
...
def configure_routes(app):
    ...
    from apollos.routers.api_content import api_content
    from apollos.routers.api_memories import api_memories
    from apollos.routers.api_model import api_model
    from apollos.routers.notion import notion_router
    ...
    if is_twilio_enabled():
        from apollos.routers.api_phone import api_phone
