...
from apollos.processor.conversation.google.utils import _is_retryable_error
from apollos.processor.conversation.utils import get_image_from_base64, get_image_from_url
from apollos.routers.helpers import ChatEvent, ImageShape, generate_better_image_prompt
from apollos.routers.storage import upload_generated_image_to_bucket
from apollos.utils import state
from apollos.utils.helpers import convert_image_to_webp, is_none_or_empty, timer
...
async def text_to_image(
    ...
    for chat in chat_history[-4:]:
        ...
        elif chat.by == "apollos" and chat.images and chat.intent and chat.intent.inferred_queries:
            image_chat_history += [
                ChatMessageModel(by=chat.by, message=chat.intent.inferred_queries[0], intent=default_intent)
            ]
        elif chat.by == "apollos" and chat.intent and chat.intent.type in ["remember", "reminder"]:
            image_chat_history += [ChatMessageModel(by=chat.by, message=chat.message, intent=default_intent)]
...
def generate_image_with_replicate(
    ...
    seed = int(os.getenv("APOLLOS_LLM_SEED")) if os.getenv("APOLLOS_LLM_SEED") else None
    if seed:
...
def format_messages_for_gemini(
    ...
    for chat in chat_history:
        role = "model" if chat.by == "apollos" else "user"
        parts = []
