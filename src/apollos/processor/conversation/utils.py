...
from apollos.search_filter.base_filter import BaseFilter
...
from apollos.utils.rawconfig import FileAttachment
...
def construct_chat_history_for_operator(conversation_history: List[ChatMessageModel], n: int = 6) -> list[AgentMessage]:
    ...
    Only include last n completed turns (i.e with user and apollos message).
    """
    ...
    for chat in conversation_history:
        ...
        elif chat.by == "apollos" and chat.message:
...
def message_to_log(
    ...
    apollos_message_metadata={},
    chat_history: List[ChatMessageModel] = [],
) -> List[ChatMessageModel]:
    """Create json logs from messages, metadata for conversation log"""
    default_apollos_message_metadata = {
        "intent": {"type": "remember", "memory-type": "notes", "query": user_message},
    }
    apollos_response_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    apollos_message_metadata = {k: v for k, v in apollos_message_metadata.items() if v is not None}

    # Create json log from Human's message
    human_log = merge_dicts({"message": user_message, "by": "you"}, user_message_metadata)

    # Create json log from GPT's response
    apollos_log = merge_dicts(apollos_message_metadata, default_apollos_message_metadata)
    apollos_log = merge_dicts(
        {"message": chat_response, "by": "apollos", "created": apollos_response_time}, apollos_log
    )
    ...
    try:
        StrictChatMessageModel(**apollos_log)
    except ValidationError as e:
        logger.error(f"Validation error in apollos chat message: {e}\nApollos Message: {apollos_log}\n")

    human_message = ChatMessageModel(**human_log)
    apollos_message = ChatMessageModel(**apollos_log)
    chat_history.extend([human_message, apollos_message])
    return chat_history
...
async def save_to_conversation_log(
    ...
    from apollos.routers.helpers import ai_update_memories
    ...
    if generated_mermaidjs_diagram:
        apollos_message_metadata["mermaidjsDiagram"] = generated_mermaidjs_diagram
    ...
    try:
        new_messages = message_to_log(
            ...
            apollos_message_metadata=apollos_message_metadata,
            chat_history=[],
        )
    except ValidationError as e:
        new_messages = None
...
def generate_chatml_messages_with_context(
    ...
    for chat in chat_history:
        ...
        if chat.by == "apollos" and chat.intent and "excalidraw" in chat.intent.type:
        ...
        # Add user query with attached file, images or apollos response
        if chat.queryFiles:
            raw_query_files = chat.queryFiles
