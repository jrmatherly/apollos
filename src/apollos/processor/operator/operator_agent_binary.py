...
from apollos.database.models import ChatMessageModel, ChatModel
from apollos.processor.conversation.utils import (
    AgentMessage,
    OperatorRun,
    construct_structured_message,
)
from apollos.processor.operator.grounding_agent import GroundingAgent
from apollos.processor.operator.grounding_agent_uitars import GroundingAgentUitars
from apollos.processor.operator.operator_actions import OperatorAction, WaitAction
from apollos.processor.operator.operator_agent_base import AgentActResult, OperatorAgent
from apollos.processor.operator.operator_environment_base import (
    EnvironmentType,
    EnvState,
    EnvStepResult,
)
from apollos.routers.helpers import send_message_to_model_wrapper
from apollos.utils.helpers import get_openai_async_client, is_none_or_empty
...
class BinaryOperatorAgent(OperatorAgent):
    ...
    def _format_message_for_api(self, messages: list[AgentMessage]) -> List[ChatMessageModel]:
        """Format operator agent messages into the Apollos conversation history format."""
        formatted_messages = [
            ChatMessageModel(
    ...
    def get_instruction(self, environment_type: EnvironmentType, env_state: EnvState) -> str:
        ...
        if environment_type == EnvironmentType.BROWSER:
            return dedent(
                f"""
                # Introduction
                * You are Apollos, a smart and resourceful web browsing assistant. You help the user accomplish their task using a web browser.
            ...
            return dedent(
                f"""
                # Introduction
                * You are Apollos, a smart and resourceful computer assistant. You help the user accomplish their task using a computer.
