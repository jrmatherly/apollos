# Apollos Operator (Experimental)

## Overview
Give Apollos its own computer to operate in a transparent, controlled manner. Accomplish tasks that require visual browsing, file editing and terminal access. Operator with research mode can work for 30+ minutes to accomplish more substantial tasks like feature development, travel planning, shopping etc.
...
### Installation Steps
1. Download the Apollos docker-compose.yml file
...
2. Update the `docker-compose.yml` to enable computer operator
    ...
    - Uncomment `APOLLOS_OPERATOR_ENABLED=True` to enable the operator tool
    - Uncomment `- /var/run/docker.sock:/var/run/docker.sock` to mount docker socket to allow apollos to operate its computer container.

3. Start Apollos services
...
## Usage
Use the `/operator` command or ask Apollos in normal or research mode to use the operator tool to have it operate its computer:

**Examples:**
- `/operator Find flights from Bangkok to Mexico City with no US layover`
- `/research Clone the apollos repo and tell me how the operator tool is implemented`
