    ...
    <Tabs groupId="operating-systems" queryString="os">
      <TabItem value="macos" label="MacOS">
        ...
        1. Download the Apollos docker-compose.yml file [from Github](https://github.com/jrmatherly/apollos/blob/master/docker-compose.yml)
           ```shell
           mkdir ~/.apolloslos && cd apollospollos
           wget https://raw.githubusercontent.com/apolloslos-apollospollos/master/docker-compose.yml
           ```
      ...
      <h3>Setup</h3>
      1. Download the Apollos docker-compose.yml file [from Github](https://github.com/apolloslos-apollospollos/blob/master/docker-compose.yml)
         ```shell
...
Run the following command in your terminal to install the Apollos server.
  ...
  <Tabs groupId="operating-systems" queryString="os">
    ...
    ```shell
CMAKE_ARGS="-DGGML_METAL=on" python -m pip install 'apolloslos[local]'
    ```
        </TabItem>
        <TabItem value="intel" label="Intel">
    ```shell
python -m pip install 'apolloslos[local]'
    ```
        </TabItem>
      </Tabs>
    </TabItem>
    <TabItem value="windows" label="Windows">
      ...
      <Tabs groupId="gpu" queryString="gpu">
       ...
       ```shell
 # Install Apollos
 py -m pip install 'apolloslos[local]'
      ```
        </TabItem>
 ...
 py -m pip install 'apolloslos[local]'
    ...
    <TabItem value="linux" label="Linux">
      <Tabs groupId="gpu" queryString="gpu">
        ...
        <TabItem value="nvidia" label="NVIDIA (CUDA) GPU">
          ```shell
          CMAKE_ARGS="-DGGML_CUDA=on" FORCE_CMAKE=1 python -m pip install 'apolloslos[local]'
          ```
        </TabItem>
        <TabItem value="amd" label="AMD (ROCm) GPU">
          ```shell
          CMAKE_ARGS="-DGGML_HIPBLAS=on" FORCE_CMAKE=1 python -m pip install 'apolloslos[local]'
          ```
        </TabItem>
        <TabItem value="vulkan" label="VULKAN GPU">
          ```shell
          CMAKE_ARGS="-DGGML_VULKAN=on" FORCE_CMAKE=1 python -m pip install 'apolloslos[local]'
          ```
        </TabItem>
      </Tabs>
    </TabItem>
...
```shell
USE_EMBEDDED_DB="true" apolloslos --anonymous-mode
```
...
3. Your setup is complete once you see `🌖 Apollos is ready to engage` in the server logs on your terminal!


:::tip[Auto Start]
To start Apollos automatically in the background use [Task scheduler](https://www.windowscentral.com/how-create-automated-task-using-task-scheduler-windows-10) on Windows or [Cron](https://en.wikipedia.org/wiki/Cron) on Mac, Linux (e.g. with `@reboot apolloslos`)
:::
  </TabItem>
</Tabs>
...
Setup which chat model you'd want to use. Apollos supports local and online chat models.
...
### Upgrade Server
  <Tabs groupId="server" queryString>
    <TabItem value="pip" label="Pip">
      ```shell
      pip install --upgrade apolloslos
      ```
      *Note: To upgrade to the latest pre-release version of the apolloslos server run below command*
    </TabItem>
    <TabItem value="docker" label="Docker">
      ...
      ```shell
      # Windows users should use their WSL2 terminal to run these commands
      cd ~/.apolloslos  # assuming yoapollospollos docker-compose.yml file is here
...
### Upgrade Clients
  <Tabs groupId="client" queryString>
    <TabItem value="desktop" label="Desktop">
      ...
      - You can manually download the latest version from the [Apollos Website](https://apolloslos.dev/downloads).
    </TabItem>
    <TabItem value="emacs" label="Emacs">
    - Use your Emacs Package Manager to Upgrade
    - See [apolloslos.el package setup](/clients/emacs#setup) for details
    </TabItem>
    <TabItem value="obsidian" label="Obsidian">
      - Upgrade via the Community plugins tab on the settings pane in the Obsidian app
      - See the [apolloslos plugin setup](/clients/obsidian#setup) for details
    </TabItem>
  </Tabs>
...
### Uninstall Server
  <Tabs groupId="server" queryString>
    <TabItem value="pip" label="Pip">
    ```shell
    # uninstall apolloslos server
    pip uninstall apolloslos

    # delete apolloslos postgres db
    dropdb apolloslos -U postgres
    ```
    </TabItem>
    <TabItem value="docker" label="Docker">
...
- **Process**:
  ...
  2. Use `pipx` to install Apollos to avoid dependency conflicts with other python packages.
     ```shell
     pipx install apolloslos
     ```
  3. Now start `apolloslos` using the standard steps described earlier

#### Install fails while building Tokenizer dependency
- **Details**: `pip install apolloslos` fails while building the `tokenizers` dependency. Complains about Rust.
- **Fix**: Install Rust to build the tokenizers package. For example on Mac run:
    ...
    source ~/.cargo/env
    ```
- **Refer**: [Issue with Fix](https://github.com/apolloslos-apollospollos/issues/82#issuecomment-1241890946) for more details
