...
from apollos.database.models import Entry, GithubConfig, ApollosUser
...
from apollos.processor.content.text_to_entries import TextToEntries
...
def test_text_search_setup_with_empty_file_creates_no_entries(search_config, default_user: ApollosUser):
    # Arrange
    initial_data = {
...
# ----------------------------------------------------------------------------------------------------
@pytest.mark.django_db
def test_text_indexer_deletes_embedding_before_regenerate(search_config, default_user: ApollosUser, caplog):
    # Arrange
    data = {
...
# ----------------------------------------------------------------------------------------------------
@pytest.mark.django_db
def test_text_index_same_if_content_unchanged(search_config, default_user: ApollosUser, caplog):
    # Arrange
...
async def test_text_search(search_config):
    # Arrange
    default_user, _ = await ApollosUser.objects.aget_or_create(
...
# ----------------------------------------------------------------------------------------------------
@pytest.mark.django_db
def test_entry_chunking_by_max_tokens(tmp_path, search_config, default_user: ApollosUser, caplog):
    # Arrange
...
# ----------------------------------------------------------------------------------------------------
@pytest.mark.django_db
def test_entry_chunking_by_max_tokens_not_full_corpus(tmp_path, search_config, default_user: ApollosUser, caplog):
    # Arrange
    # Insert org-mode entry with size exceeding max token limit to new org file
    data = {
        "readme.org": """
...
# ----------------------------------------------------------------------------------------------------
@pytest.mark.django_db
def test_regenerate_index_with_new_entry(search_config, default_user: ApollosUser):
    # Arrange
...
# ----------------------------------------------------------------------------------------------------
@pytest.mark.django_db
def test_update_index_with_duplicate_entries_in_stable_order(tmp_path, search_config, default_user: ApollosUser):
    # Arrange
...
# ----------------------------------------------------------------------------------------------------
@pytest.mark.django_db
def test_update_index_with_deleted_entry(tmp_path, search_config, default_user: ApollosUser):
    # Arrange
    existing_entries = list(Entry.objects.filter(user=default_user).values_list("compiled", flat=True))
...
# ----------------------------------------------------------------------------------------------------
@pytest.mark.django_db
def test_update_index_with_new_entry(search_config, default_user: ApollosUser):
    # Arrange
    # Initial indexed files
    text_search.setup(OrgToEntries, get_sample_data("org"), regenerate=True, user=default_user)
...
def test_update_index_with_deleted_file(text_to_entries: TextToEntries, search_config, default_user: ApollosUser):
...
def test_text_search_setup_github(search_config, default_user: ApollosUser):
    # Arrange
