...
# Configuration
APOLLOS_URL = os.getenv("APOLLOS_URL", "http://localhost:42110")
APOLLOS_CHAT_API_URL = f"{APOLLOS_URL}/api/chat"
...
SLEEP_SECONDS = 3 if APOLLOS_MODE == "general" else 1  # Sleep between API calls to avoid rate limiting
APOLLOS_API_TIMEOUT_SECONDS = 1200  # Default to 20 minutes
...
def index_frames_kb():
    """Index Wikipedia articles from FRAMES dataset into Apollos"""
    try:
        # Load dataset
        dataset = load_frames_kb()
        dataset_files = set(map(get_article_filename, dataset))

        # Get indexed files from Apollos API
        headers = {"Authorization": f"Bearer {APOLLOS_API_KEY}"} if APOLLOS_API_KEY else {}
        try:
            response = requests.get(f"{APOLLOS_URL}/api/content/computer", headers=headers)
        ...
        for batch in tqdm(batcher(filtered_dataset, batch_size), total=total_batches, desc="Indexing FRAMES KB"):
            ...
            try:
                response = requests.patch(f"{APOLLOS_URL}/api/content?client=eval", headers=headers, files=files)
                response.raise_for_status()
...
def get_agent_response(prompt: str) -> Dict[str, Any]:
    """Get response from the Apollos API"""
    # Set headers
    headers = {"Content-Type": "application/json"}
    if not is_none_or_empty(APOLLOS_API_KEY):
        headers["Authorization"] = f"Bearer {APOLLOS_API_KEY}"

    try:
        response = requests.post(
            APOLLOS_CHAT_API_URL,
            headers=headers,
            json={
                "q": prompt,
                "create_new": True,
            },
            timeout=APOLLOS_API_TIMEOUT_SECONDS,
        )
        response.raise_for_status()
...
def evaluate_response_for_ir(
    ...
    """Evaluate Apollos response against benchmark ground truth using string matching"""
    try:
...
def process_batch(batch, batch_start, results, dataset_length, response_evaluator):
    ...
    for idx, (prompt, answer, reasoning_type) in enumerate(batch):
        ...
        prompt = f"/{APOLLOS_MODE} {prompt}" if APOLLOS_MODE and not prompt.startswith(f"/{APOLLOS_MODE}") else prompt
...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate Apollos on a supported benchmark.")
    parser.add_argument(
        "--output",
        "-o",
        default=None,
...
if __name__ == "__main__":
    ...
    with timer("Ran eval script in", logger, log_level=logging.INFO):
        main()
    logger.info(f"{datetime.now()} - End Quizzing Apollos.")
