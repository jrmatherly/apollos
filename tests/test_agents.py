...
from apollos.database.models import Agent, ChatModel, Entry, FileObject, ApollosUser
from apollos.routers.helpers import execute_search
from apollos.utils.helpers import get_absolute_path
from tests.helpers import ChatModelFactory


def test_create_default_agent(default_user: ApollosUser):
    ChatModelFactory()
...
async def test_create_or_update_agent(default_user: ApollosUser, default_openai_chat_model_option: ChatModel):
...
async def test_create_or_update_agent_with_knowledge_base(
    default_user2: ApollosUser, default_openai_chat_model_option: ChatModel, chat_client
):
...
async def test_agent_with_knowledge_base_and_search_not_creator(
    default_user2: ApollosUser, default_openai_chat_model_option: ChatModel, chat_client, default_user3: ApollosUser
):
    full_filename = get_absolute_path("tests/data/markdown/having_kids.markdown")
    new_agent = await AgentAdapters.aupdate_agent(
        default_user2,
...
async def test_large_knowledge_base_atomic_update(
    default_user2: ApollosUser, default_openai_chat_model_option: ChatModel, chat_client_with_large_kb
):
    """
